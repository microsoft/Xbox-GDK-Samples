...
namespace
{
    std::unique_ptr<OfflineRT> g_offlineRT;
...
int SampleMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE, _In_ LPWSTR lpCmdLine, _In_ int nCmdShow)
{
    ...
    g_offlineRT = std::make_unique<OfflineRT>();
...

    {
        ...
        wcex.lpszClassName = u8"OfflineRTWindowClass";
        wcex.hbrBackground = reinterpret_cast<HBRUSH>(COLOR_WINDOW + 1);
        if (!RegisterClassExA(&wcex))
            return 1;

        // Create window
        HWND hwnd = CreateWindowExA(0, u8"OfflineRTWindowClass", u8"OfflineRT", WS_OVERLAPPEDWINDOW,
        ...
        SetWindowLongPtr(hwnd, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(g_offlineRT.get()));
...
{
    auto offlineRT = reinterpret_cast<OfflineRT*>(GetWindowLongPtr(hWnd, GWLP_USERDATA));
    ...
    case WM_USER + 1:
        if (offlineRT)
        {
            if (wParam)
            {
                offlineRT->OnConstrained();
            }
            else
            {
                SetDisplayMode();

                offlineRT->OnUnConstrained();
            }
        }
        break;
    }
...
void SetDisplayMode() noexcept
{
    if (g_offlineRT && g_offlineRT->RequestHDRMode())
    {
        // Request HDR mode.
