    ...
    virtual void Initialize(ID3D12Device* device, ID3D12CommandQueue* /* commandQueue */) override
    {
        ...
        // Culling pass 
        {
            ...
            DX::ThrowIfFailed(device->CreateRootSignature(0,
                ...
                IID_GRAPHICS_PPV_ARGS(m_rootSignatureCull.ReleaseAndGetAddressOf())));
            ...
            DX::ThrowIfFailed(device->CreateComputePipelineState(&descPipelineState, IID_GRAPHICS_PPV_ARGS(m_pipelineStateCullGraphics.ReleaseAndGetAddressOf())));
        ...
        // Culling pass 
        {
            ...
            DX::ThrowIfFailed(device->CreateComputePipelineState(&descPipelineState, IID_GRAPHICS_PPV_ARGS(m_pipelineStateCullCompute.ReleaseAndGetAddressOf())));
        ...
        // Transform pass 
        {
            ...
            DX::ThrowIfFailed(device->CreateRootSignature(0, 
                serializedRootSignature->GetBufferPointer(), 
                serializedRootSignature->GetBufferSize(), 
                IID_GRAPHICS_PPV_ARGS(m_rootSignatureTransform.ReleaseAndGetAddressOf())));
            ...
            DX::ThrowIfFailed(device->CreateComputePipelineState(&descPipelineState, IID_GRAPHICS_PPV_ARGS(m_pipelineStateTransform.ReleaseAndGetAddressOf())));
        ...
        // Command signature graphics
        {
            ...
            DX::ThrowIfFailed(device->CreateCommandSignature(&descCommandSignature, m_rootSignatureTransform.Get(), IID_GRAPHICS_PPV_ARGS(m_commandSignatureGraphics.ReleaseAndGetAddressOf())));
        ...
        // Command signature compute
        {
            ...
            descIndirectArgument[0].Type = D3D12XBOX_INDIRECT_ARGUMENT_TYPE_DISPATCH_L2;
            descIndirectArgument[0].DispatchL2.Flags = D3D12XBOX_DISPATCH_FLAG_NONE;
            ...
            DX::ThrowIfFailed(device->CreateCommandSignature(&descCommandSignature, m_rootSignatureTransform.Get(), IID_GRAPHICS_PPV_ARGS(m_commandSignatureCompute.ReleaseAndGetAddressOf())));
        ...
        // Indirect args buffers
        {
            ...
            DX::ThrowIfFailed(
                device->CreateCommittedResource(&uploadHeapProperties,
                    ...
                    nullptr,
                    IID_GRAPHICS_PPV_ARGS(m_bufArgIn.ReleaseAndGetAddressOf())));
            ...
            auto descBufArgOut = CD3DX12_RESOURCE_DESC::Buffer(m_objectCount * sizeof(IndirectArgs), D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS | D3D12XBOX_RESOURCE_FLAG_ALLOW_INDIRECT_BUFFER);
            DX::ThrowIfFailed(
                device->CreateCommittedResource(&defaultHeapProperties,
                    ...
                    nullptr,
                    IID_GRAPHICS_PPV_ARGS(m_bufArgOut.ReleaseAndGetAddressOf())));
            ...
            DX::ThrowIfFailed(
                device->CreateCommittedResource(&uploadHeapProperties,
                    ...
                    D3D12_RESOURCE_STATE_GENERIC_READ,
                    nullptr,
                    IID_GRAPHICS_PPV_ARGS(m_bufCountIn.ReleaseAndGetAddressOf())));
            ...
            auto descBufCountOut = CD3DX12_RESOURCE_DESC::Buffer(sizeof(IndirectCount), D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS | D3D12XBOX_RESOURCE_FLAG_ALLOW_INDIRECT_BUFFER);
            DX::ThrowIfFailed(
                device->CreateCommittedResource(&defaultHeapProperties,
                    D3D12_HEAP_FLAG_NONE,
                    &descBufCountOut,
                    D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
                    nullptr,
                    IID_GRAPHICS_PPV_ARGS(m_bufCountOut.ReleaseAndGetAddressOf())));
        ...
        // Transform buffers
        {
            ...
            DX::ThrowIfFailed(device->CreateCommittedResource(&uploadHeapProperties,
                ...
                D3D12_RESOURCE_STATE_GENERIC_READ,
                nullptr,
                IID_GRAPHICS_PPV_ARGS(m_bufTransform.ReleaseAndGetAddressOf())));
        ...
        {
            // On PC we'd need to copy to a shader visible heap, but here, we don't bother
            D3D12_DESCRIPTOR_HEAP_DESC uavHeapDesc = {};
            uavHeapDesc.NumDescriptors = 2;
            uavHeapDesc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
            uavHeapDesc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;
            DX::ThrowIfFailed(
                device->CreateDescriptorHeap(&uavHeapDesc, IID_GRAPHICS_PPV_ARGS(m_descriptorHeapUav.ReleaseAndGetAddressOf())));
            ...
            // Implementation dependent: This is where the buffer data is stored
            auto addrArgOut = reinterpret_cast<UINT64*>(m_hCpuDescriptorUavArgOut.GetWritePointerX())[0];
            addrArgOut &= 0x000000ffffffffff;
            auto bufArgOutMemory = reinterpret_cast<void*>(addrArgOut);

            auto addrCountOut = reinterpret_cast<UINT64*>(m_hCpuDescriptorUavCountOut.GetWritePointerX())[0];
            addrCountOut &= 0x000000ffffffffff;
            auto bufArgCountMemory = reinterpret_cast<void*>(addrCountOut);

            // Pretend previous contents of arg memory were very large uints
            if (bufArgOutMemory)
            {
#pragma warning(suppress : 6387) // bufArgOutMemory is null-checked above
                memset(bufArgOutMemory, 0xff, m_objectCount * sizeof(IndirectArgs));
            }

            // Pretend previous contents of count memory were not 0
            if (bufArgCountMemory)
            {
#pragma warning(suppress : 6387) // bufArgCountMemory is null-checked above
                memset(bufArgCountMemory, 0xff, sizeof(IndirectCount));
            }
        }
    }
    ...
    virtual void Render(ID3D12Device* /* device */, ID3D12CommandQueue* commandQueue, bool hang) override
    {
        ...
        auto maxObjectCount = async ? 1U : m_objectCount;           // async only supports a max object count of 1
        auto bufCountOut = async ? nullptr : m_bufCountOut.Get();   // async doesn't support a count buffer
