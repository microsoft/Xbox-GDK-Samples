...
public:
    ...
    virtual void Initialize(ID3D12Device* device, ID3D12CommandQueue* commandQueue) override
    {
        ...
        // VS - GS linkage error: Signatures between stages are incompatible. The input stage requires Semantic/Index (SV_InstanceID,0) as input, but it is not provided by the output stage. [ STATE_CREATION ERROR #666 ]
        SCOPED_ERROR_FILTER(device, 0xE5294B1F);
        ...
        // Create the scale factors buffer
        {
            ...
            DX::ThrowIfFailed(
                device->CreateCommittedResource(&uploadHeapProperties,
                    ...
                    nullptr,
                    IID_GRAPHICS_PPV_ARGS(m_bufScale.ReleaseAndGetAddressOf())));
...
#else
    static const uint32_t                                   m_primitiveCount = 100000;
#endif
};
