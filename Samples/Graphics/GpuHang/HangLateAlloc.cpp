...
class HangLateAlloc final : public Hang
{
public:
    HangLateAlloc() :
        Hang(),
    ...
    virtual ~HangLateAlloc() override {}
    ...
    virtual const std::vector<const wchar_t*> GetDescription() const override
    {
        ...
        description.push_back(L"This hang occurs if the \"late alloc\" setting is too high. ");
        description.push_back(L"Late alloc is a feature by which vertex waves are speculatively launched  ");
        description.push_back(L"before there is enough parameter cache space to hold their outputs. ");
        description.push_back(L"Above a certain number of waves, it's possible for all CUs to be occupied by VS waves, ");
        description.push_back(L"leaving insufficient resources to run PS waves and free up parameter cache space. ");
    ...
    virtual void Initialize(ID3D12Device* device, ID3D12CommandQueue* commandQueue) override
    {
        ...
        auto vertexShaderBlob = DX::ReadData(L"LateAllocVs.cso");

        auto pixelShaderBlob = DX::ReadData(L"LateAllocPs.cso");
        ...
        {
            SCOPED_ERROR_FILTER(device, 0xAD736EA7);    // MaxWavesWithLateAllocParameterCache is set too high for safe use without VS masking.

            // Extended pipeline state with higher-than-safe late alloc setting
            D3D12XBOX_GRAPHICS_SHADER_LIMITS_DESC shaderLimitsDesc =
            {
            };
            D3D12XboxInitializeDefaultGraphicsShaderLimits(&shaderLimitsDesc);
            shaderLimitsDesc.MaxWavesWithLateAllocParameterCache = D3D12XBOX_SHADER_MAX_WAVES_WITH_LATE_ALLOC_PARAMETER_CACHE_MAX;

            // For Scorpio, restrict to the Durango number of CUs (otherwise, we avoid the hang)
            if (IsScorpioClass(device))
            {
                shaderLimitsDesc.VSDisableFlags =
                shaderLimitsDesc.PSDisableFlags =
                    D3D12XBOX_SHADER_UNIT_DISABLE_FLAG_CU_6
                    | D3D12XBOX_SHADER_UNIT_DISABLE_FLAG_CU_7
                    | D3D12XBOX_SHADER_UNIT_DISABLE_FLAG_CU_8
                    | D3D12XBOX_SHADER_UNIT_DISABLE_FLAG_CU_9;
            }

            D3D12XBOX_EXTENDED_GRAPHICS_PIPELINE_STATE_DESC extendedDescs[] =
            {
                {D3D12XBOX_EXTENDED_GRAPHICS_PIPELINE_STATE_SHADER_LIMITS, },
            };
            extendedDescs[0].ShaderLimitsDesc = shaderLimitsDesc;

            SCOPED_ERROR_FILTER(device, 0x471F52F6);  // ID3D12Device::CreateGraphicsPipelineStateX: D3D12XBOX_GRAPHICS_SHADER_LIMITS_DESC: VSDisableFlags is invalid.
            DX::ThrowIfFailed(device->CreateGraphicsPipelineStateX(&psoDesc, _countof(extendedDescs), extendedDescs, IID_GRAPHICS_PPV_ARGS(m_pipelineStateLateAlloc.ReleaseAndGetAddressOf())));
        }
        ...
        {
            // Downscale the usual rectangle by 100.0f, to balance VS and PS waves
            static const Vertex s_vertexData[4] =
            {
                { { -0.5f, -0.5f, 0.5f, 100.0f },{ 0.f, 1.f } },
                { { 0.5f, -0.5f, 0.5f, 100.0f },{ 1.f, 1.f } },
                { { 0.5f,  0.5f, 0.5f, 100.0f },{ 1.f, 0.f } },
                { { -0.5f,  0.5f, 0.5f, 100.0f },{ 0.f, 0.f } },
            };
    ...
    virtual void Uninitialize(ID3D12Device* /* device */) override
    {
        ...
        m_pipelineStateLateAlloc = nullptr;
    ...
    virtual void Render(ID3D12Device* /* device */, ID3D12CommandQueue* commandQueue, bool hang) override
    {
        ...
        if (hang)
        {
            m_commandList->SetPipelineState(m_pipelineStateLateAlloc.Get());
        }
        else
        {
        ...
        // Draw quad.
        m_commandList->DrawIndexedInstanced(6, m_instanceCount, 0, 0, 0);
...
private:
    ...
    Microsoft::WRL::ComPtr<ID3D12PipelineState>             m_pipelineStateLateAlloc;
    ...
    // Large number of instances, to generate many VS waves
    const uint32_t                                          m_instanceCount = 1U << 16U;
};

static HangLateAlloc hang;
