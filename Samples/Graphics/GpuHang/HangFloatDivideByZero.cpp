...
public:
    ...
    virtual ~HangFloatDivideByZero() override {}
    ...
    virtual const std::vector<const wchar_t*> GetDescription() const override
    {
        ...
        description.push_back(L"This hang occurs if a shader performs a float divide by zero.");
    ...
    virtual void Initialize(ID3D12Device* device, ID3D12CommandQueue* /* commandQueue */) override
    {
        ...
        auto computeShaderBlob = DX::ReadData(L"DivideCs.cso");
        ...
        auto descBufInputNumerator = CD3DX12_RESOURCE_DESC::Buffer(m_bufSize);
        DX::ThrowIfFailed(
            device->CreateCommittedResource(&uploadHeapProperties,
                D3D12_HEAP_FLAG_NONE,
                &descBufInputNumerator,
                D3D12_RESOURCE_STATE_GENERIC_READ,
                nullptr,
                IID_GRAPHICS_PPV_ARGS(m_bufInputNumerator.ReleaseAndGetAddressOf())));

        void* bufDataNumerator = nullptr;
        CD3DX12_RANGE readRangeNumerator(0, 0);		// We do not intend to read from this resource on the CPU.
        DX::ThrowIfFailed(m_bufInputNumerator->Map(0, &readRangeNumerator, &bufDataNumerator));
        auto bufDataNumeratorFloat = reinterpret_cast<float*>(bufDataNumerator);
        for (auto i = 0U; i < m_bufSize / sizeof(float); ++i)
        {
            bufDataNumeratorFloat[i] = 1.0f;
        }
        m_bufInputNumerator->Unmap(0, nullptr);

        auto descBufInputDenominatorNonZero = CD3DX12_RESOURCE_DESC::Buffer(m_bufSize);
        DX::ThrowIfFailed(
            device->CreateCommittedResource(&uploadHeapProperties,
                D3D12_HEAP_FLAG_NONE,
                &descBufInputDenominatorNonZero,
                D3D12_RESOURCE_STATE_GENERIC_READ,
                nullptr,
                IID_GRAPHICS_PPV_ARGS(m_bufInputDenominatorNonZero.ReleaseAndGetAddressOf())));

        void* bufDataDenominatorNonZero = nullptr;
        CD3DX12_RANGE readRangeDenominatorNonZero(0, 0);		// We do not intend to read from this resource on the CPU.
        DX::ThrowIfFailed(m_bufInputDenominatorNonZero->Map(0, &readRangeDenominatorNonZero, &bufDataDenominatorNonZero));
        auto bufDataDenominatorNonZeroFloat = reinterpret_cast<float*>(bufDataDenominatorNonZero);
        for (auto i = 0U; i < m_bufSize / sizeof(float); ++i)
        {
            bufDataDenominatorNonZeroFloat[i] = 1.0f;
        }
        m_bufInputDenominatorNonZero->Unmap(0, nullptr);

        auto descBufInputDenominatorZero = CD3DX12_RESOURCE_DESC::Buffer(m_bufSize);
        DX::ThrowIfFailed(
            device->CreateCommittedResource(&uploadHeapProperties,
                D3D12_HEAP_FLAG_NONE,
                &descBufInputDenominatorZero,
                D3D12_RESOURCE_STATE_GENERIC_READ,
                nullptr,
                IID_GRAPHICS_PPV_ARGS(m_bufInputDenominatorZero.ReleaseAndGetAddressOf())));

        void* bufDataDenominatorZero = nullptr;
        CD3DX12_RANGE readRangeDenominatorZero(0, 0);		// We do not intend to read from this resource on the CPU.
        DX::ThrowIfFailed(m_bufInputDenominatorZero->Map(0, &readRangeDenominatorZero, &bufDataDenominatorZero));
        auto bufDataDenominatorZeroFloat = reinterpret_cast<float*>(bufDataDenominatorZero);
        for (auto i = 0U; i < m_bufSize / sizeof(float); ++i)
        {
            bufDataDenominatorZeroFloat[i] = 0.0f;
        }
        m_bufInputDenominatorZero->Unmap(0, nullptr);
    ...
    virtual void Uninitialize(ID3D12Device* /* device */) override
    {
        ...
        m_bufInputNumerator = nullptr;
        m_bufInputDenominatorNonZero = nullptr;
        m_bufInputDenominatorZero = nullptr;
        m_bufOutput = nullptr;
    }
    ...
    virtual void Render(ID3D12Device* /*device*/, ID3D12CommandQueue* commandQueue, bool hang) override
    {
        ...
        auto bufInput = hang ? m_bufInputDenominatorZero : m_bufInputDenominatorNonZero;

        commandList->SetComputeRootSignature(m_rootSignature.Get());
        commandList->SetPipelineState(m_pipelineState.Get());

        commandList->SetComputeRootShaderResourceView(0, m_bufInputNumerator->GetGPUVirtualAddress());
        commandList->SetComputeRootShaderResourceView(1, bufInput->GetGPUVirtualAddress());

        commandList->SetComputeRootUnorderedAccessView(2, m_bufOutput->GetGPUVirtualAddress());
...
private:
    ...
    Microsoft::WRL::ComPtr<ID3D12Resource>                  m_bufInputNumerator;
    Microsoft::WRL::ComPtr<ID3D12Resource>                  m_bufInputDenominatorNonZero;
    Microsoft::WRL::ComPtr<ID3D12Resource>                  m_bufInputDenominatorZero;
    Microsoft::WRL::ComPtr<ID3D12Resource>                  m_bufOutput;

    static const uint32_t                                   m_bufSize = 1024U;
};

static HangFloatDivideByZero hang;
