...
class HangShaderAssert final : public Hang
{
public:
    HangShaderAssert() :
        Hang(
    ...
    virtual const std::vector<const wchar_t*> GetDescription() const override
    {
        ...
        description.push_back(L"This hang occurs if a shader fires an assert. ");
        description.push_back(L"A shader can fire an assert using __XB_DebugBreak if certain conditions in the shader fail. ");
    ...
    virtual void Initialize(ID3D12Device* device, ID3D12CommandQueue* /*commandQueue*/) override
    {
        ...
        auto computeShaderBlob = DX::ReadData(L"AssertCs.cso");
        ...
        {
            // Initialize all to values in the range 0-100.
            // Later, we will assert that the values are in this range.
            auto const elementCount = 256;
            Input input[elementCount] = { 0 };
            for (auto i = 0U; i < elementCount; ++i)
            {
                input[i].value = rand() % 100U;
            }
            ...
            auto descBufOutput = CD3DX12_RESOURCE_DESC::Buffer(sizeof(Output[elementCount]), D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
            DX::ThrowIfFailed(
    ...
    virtual void Render(ID3D12Device* /* device */, ID3D12CommandQueue* commandQueue, bool hang) override
    {
        ...
        ID3D12CommandQueue* queue = async ? m_commandQueueCompute.Get() : commandQueue;
        ID3D12CommandAllocator* commandAllocator = async ? m_commandAllocatorCompute.Get() : m_commandAllocatorGraphics.Get();
        ID3D12GraphicsCommandList* commandList = async ? m_commandListCompute.Get() : m_commandListGraphics.Get();

        PIXBeginRetailEvent(commandList, PIX_COLOR_DEFAULT, __FUNCTIONW__);
        ...
        if (hang)
        {
            ...
            reinterpret_cast<Input*>(bufData)[128].value = 0xff;
            m_bufInput->Unmap(0, nullptr);
        }
        ...
        queue->ExecuteCommandLists(1, CommandListCast(&commandList));
