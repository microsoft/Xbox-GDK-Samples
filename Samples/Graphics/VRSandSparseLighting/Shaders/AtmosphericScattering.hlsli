...
#define G                       -0.95          // G is ordinarily between -0.75 and -0.999 Negative values of G scatter more light in a forward direction, positive values scatter light back towards the light source
#define G2                      (G * G)

#define SKY_COLOUR_L            (float3(0.07, 0.12, 0.25))
#define SKY_COLOUR_H            (float3(0.09, 0.17, 0.4) * 2.0)

#define DISC_COLOUR_H           float3(0.04, 0.02, 0.02)
#define DISC_COLOUR_L           float3(0.08, 0.04, 0.01)


float CalcInvZenithAngle(float dirY, float offset = 0.125)
{
...
float3 AtmosphericScattering(float attenuation, float mieAttenuation, float cosAngle, float lightDirY, out float3 extinction)
{
    ...
    phase *= 3.0 * (mieAttenuation * (1.0 - G2)) / (2.0 * (2.0 + G2));                  // equations give 3x(1-g^2) / 2x(2 + g^2)

    float3 phaseColour = phase * (discColour / skyColour);

    // boost into HDR range, especially when looking at the sun
    float hdrExtra = (0.3 + saturate(lightDirY * 0.7)) * (1.0 + pow(cosAngle, 16.0));
