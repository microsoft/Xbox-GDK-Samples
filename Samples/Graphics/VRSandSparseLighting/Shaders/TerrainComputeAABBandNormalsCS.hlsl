...
void WriteNormalToUAV(uint sliceIndex, uint2 coord, float2 normal)
{
    NormalMapUAV[sliceIndex][coord] = normal;
}
    ...
    uint groupThreadIndex : SV_GroupIndex)
{
	...
	{
        float2 uv = ((Gid * GROUP_DIM) + (2 * GTid.xy)) * invSourceHeightMapSize + halfTexelHeightMapSize;
	...
	*/	
    ...
    WriteNormalToUAV(0, destCoord, normalScaled);
    ...
    if (WaveIsFirstLane()) 
    {
        ...
        uint destAddress = TerrainAABBAddress(destCoord >> sourceToHeightMapSizeLog2, terrainTileSizeLog2, terrainTileCountPerAxis);
    ...
    for (uint i = 1; i < 4; ++i)
    {
        ...
        if (((GTid.x | GTid.y) & mask) == 0)
        {
            ...
            WriteNormalToUAV(i, ((Gid * GROUP_DIM) + GTid.xy) >> i, n);
