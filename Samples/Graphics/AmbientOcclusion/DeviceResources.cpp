...
//
// Customized with XG layout information
//
...
DeviceResources::DeviceResources(
    ...
    unsigned int flags) noexcept(false) :
        m_backBufferIndex(0),
        m_pDepthTextureAddress(nullptr),
        m_depthBufferLayout{},
        m_depthTileMode{},
        m_fenceValue(0),
...
void DeviceResources::CreateWindowSizeDependentResources()
{
    ...
    if (m_depthBufferFormat != DXGI_FORMAT_UNKNOWN)
    {
        XG_RESOURCE_DESC depthBufferXGDesc =
        {
            ...
            1U,
            1U,
            static_cast<XG_FORMAT>(m_depthBufferFormat),
            { 1U, 0U, },
            XG_TEXTURE_LAYOUT_UNKNOWN,
            XG12_RESOURCE_MISC_ALLOW_DEPTH_STENCIL,
        };

        // retrieve tile modes
#ifdef _GAMING_XBOX_SCARLETT
#if _GXDK_VER >= 0x6336073E // October 2023 GDK
        XGSuggestSwizzleMode(&depthBufferXGDesc, XG_SUGGEST_SWIZZLE_MODE_FLAG_OPTIMIZE_FOR_PERFORMANCE, &m_depthTileMode);
#else
        XG_SWIZZLE_MODE tileModeDepth = XG_SWIZZLE_MODE_INVALID;
        XGComputeOptimalDepthStencilSwizzleMode(static_cast<XG_FORMAT>(m_depthBufferFormat), backBufferWidth, backBufferHeight, 1, 1, TRUE, FALSE, FALSE, &tileModeDepth);
        m_depthTileMode = static_cast<XG_TEXTURE_LAYOUT>(0x100 | tileModeDepth);
#endif
#else
        XG_TILE_MODE tileModeDepth = XG_TILE_MODE_INVALID, tileModeStencil = XG_TILE_MODE_INVALID;
        XGComputeOptimalDepthStencilTileModes(static_cast<XG_FORMAT>(m_depthBufferFormat), backBufferWidth, backBufferHeight, 1, 1, TRUE, FALSE, FALSE, &tileModeDepth, &tileModeStencil);
        assert(tileModeDepth == tileModeStencil);
        m_depthTileMode = static_cast<XG_TEXTURE_LAYOUT>(0x100 | tileModeDepth);
#endif

        depthBufferXGDesc.Layout = m_depthTileMode;

        ComPtr<XGTextureAddressComputer> computer;
        DX::ThrowIfFailed(XGCreateTextureComputer(&depthBufferXGDesc, computer.GetAddressOf()));

        m_depthBufferLayout = {};
        DX::ThrowIfFailed(computer->GetResourceLayout(&m_depthBufferLayout));

        // Reserve virtual address
        DWORD allocType = MEM_2MB_PAGES | MEM_RESERVE | MEM_COMMIT;

        m_pDepthTextureAddress = XMemVirtualAlloc(nullptr,
            m_depthBufferLayout.SizeBytes,
            allocType,
            XMEM_GRAPHICS, PAGE_READWRITE | PAGE_GRAPHICS_READWRITE | PAGE_WRITECOMBINE);

        if (!m_pDepthTextureAddress)
        {
            throw std::bad_alloc();
        }
        ...
        DX::ThrowIfFailed(m_d3dDevice->CreatePlacedResourceX(
            *reinterpret_cast<D3D12_GPU_VIRTUAL_ADDRESS*>(&m_pDepthTextureAddress),
            depthBufferD3D12Desc,
            D3D12_RESOURCE_STATE_DEPTH_WRITE,
            &depthOptimizedClearValue,
            IID_GRAPHICS_PPV_ARGS(m_depthStencil.GetAddressOf())));
