...
//
// This has been customized to include handling of a compute command queue.
//
...
void DeviceResources::CreateDeviceResources()
{
    ...
    ThrowIfFailed(m_d3dDevice->CreateCommandQueue(&computeQueueDesc, IID_GRAPHICS_PPV_ARGS(m_computeCommandQueue.ReleaseAndGetAddressOf())));

    m_computeCommandQueue->SetName(L"ComputeDeviceResources");
    ...
    for (UINT n = 0; n < m_backBufferCount; n++)
    {
        ThrowIfFailed(m_d3dDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_COMPUTE, IID_GRAPHICS_PPV_ARGS(m_computeCommandAllocators[n].ReleaseAndGetAddressOf())));

        wchar_t name[25] = {};
        swprintf_s(name, L"Compute %u", n);
        m_computeCommandAllocators[n]->SetName(name);
    }
    ...
    ThrowIfFailed(m_d3dDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_COMPUTE, m_computeCommandAllocators[0].Get(), nullptr, IID_GRAPHICS_PPV_ARGS(m_computeCommandList.ReleaseAndGetAddressOf())));
    ...
    ThrowIfFailed(m_d3dDevice->CreateFence(m_computeToGraphicsFenceValues, D3D12_FENCE_FLAG_NONE, IID_GRAPHICS_PPV_ARGS(m_computeToGraphicsFence.ReleaseAndGetAddressOf())));
    m_computeToGraphicsFenceValues++;

    m_computeToGraphicsFence->SetName(L"ComputeDeviceResources");

    ThrowIfFailed(m_d3dDevice->CreateFence(m_graphicsToComputeFenceValues, D3D12_FENCE_FLAG_NONE, IID_GRAPHICS_PPV_ARGS(m_graphicsToComputeFence.ReleaseAndGetAddressOf())));
    m_graphicsToComputeFenceValues++;

    m_graphicsToComputeFence->SetName(L"ComputeDeviceResources");
...
void DeviceResources::Prepare(D3D12_RESOURCE_STATES beforeState, D3D12_RESOURCE_STATES afterState)
{
    ...
    ThrowIfFailed(m_computeCommandAllocators[m_backBufferIndex]->Reset());
    ThrowIfFailed(m_computeCommandList->Reset(m_computeCommandAllocators[m_backBufferIndex].Get(), nullptr));
...
void DeviceResources::Suspend()
{
    ...
    m_computeCommandQueue->SuspendX(0);
}

void DeviceResources::Resume()
{
    m_commandQueue->ResumeX();
    m_computeCommandQueue->ResumeX();
...
// Execute the command list and reset it to be reused in the same frame
void DeviceResources::ExecuteCommandListAndReuse()
{
    // Send the command list off to the GPU for processing.
    ThrowIfFailed(m_commandList->Close());
    m_commandQueue->ExecuteCommandLists(1, CommandListCast(m_commandList.GetAddressOf()));
...
// Execute the compute command list and signal so the graphics queue can wait
void DeviceResources::ExecuteComputeCommandList()
{
    ThrowIfFailed(m_computeCommandList->Close());
    m_computeCommandQueue->ExecuteCommandLists(1, CommandListCast(m_computeCommandList.GetAddressOf()));
    const UINT64 currentFenceValue = ++m_computeToGraphicsFenceValues;
    ThrowIfFailed(m_computeCommandQueue->Signal(m_computeToGraphicsFence.Get(), currentFenceValue));
}

void DeviceResources::GraphicsWaitOnCompute()
{
    m_commandQueue->Wait(m_computeToGraphicsFence.Get(), m_computeToGraphicsFenceValues);
}

void DeviceResources::ComputeWaiteOnGraphics()
{
    // Schedule a Signal command in the queue.
    const UINT64 currentFenceValue = ++m_graphicsToComputeFenceValues;
    ThrowIfFailed(m_commandQueue->Signal(m_graphicsToComputeFence.Get(), currentFenceValue));
    m_computeCommandQueue->Wait(m_graphicsToComputeFence.Get(), m_graphicsToComputeFenceValues);
}
