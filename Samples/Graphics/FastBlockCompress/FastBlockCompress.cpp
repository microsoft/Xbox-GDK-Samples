...
void Sample::Update(DX::StepTimer const& timer)
{
    ...
    case RTC_GPU:
    {
        ...
        resource1Mip->SetName(L"Top mip only");
    ...
    case RTC_CPU:
    {
        ...
        for (uint16_t i = 0; i < img->m_desc.MipLevels; ++i)
        {
            ...
            srvDesc.Texture2D.MipLevels = UINT(img->m_desc.MipLevels - i);

            device->CreateShaderResourceView(m_fbcTexture.Get(), &srvDesc, m_resourceDescriptors->GetCpuHandle(CPU_MipBase + size_t(i)));
        }
    }
    break;
    }
...
void Sample::Render()
{
    ...
    size_t indexOriginal = img->m_srv[size_t(m_mipLevel)];
    ...
    {
        rmsSubmitted = RMSError(commandList, indexOriginalBase, indexCompressBase, uint32_t(m_mipLevel), uint32_t(img->m_desc.Width), false, (img->m_compressedFormat == DXGI_FORMAT_BC5_UNORM));
    }
    ...
    DX::DrawControllerString(m_batch.get(), m_font.get(), m_ctrlFont.get(),
        L"[View] Exit   [LB] / [RB] Image select   [LThumb] Pan   [RT]/[LT] Zoom",
...
void Sample::CreateDeviceDependentResources()
{
...

    {
        ...
        m_rmsReduceBufferA->SetName(L"RMS Reduce A");
        ...
        m_rmsReduceBufferB->SetName(L"RMS Reduce B");
...

    {
        ...
        m_rmsResult->SetName(L"RMS Result");
    }
    ...
    {
        D3D12_COMMAND_QUEUE_DESC descCommandQueue = { D3D12_COMMAND_LIST_TYPE_COMPUTE, 0, D3D12_COMMAND_QUEUE_FLAG_NONE, 0u };
    ...
    m_compressedFormat(format)
{
    ...
    for (uint16_t i = 0; i < m_desc.MipLevels; ++i)
    {
        ...
        srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2D;
        srvDesc.Texture2D.MostDetailedMip = i;
        srvDesc.Texture2D.MipLevels = UINT(m_desc.MipLevels - i);
    ...
    // Offline Compressed
    {
        ...
        for (uint16_t i = 0; i < m_desc.MipLevels; ++i)
        {
            ...
            srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2D;
            srvDesc.Texture2D.MostDetailedMip = i;
            srvDesc.Texture2D.MipLevels = UINT(desc.MipLevels - i);
