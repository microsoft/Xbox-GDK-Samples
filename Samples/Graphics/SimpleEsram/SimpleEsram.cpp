...
namespace
{
    ...
    constexpr DXGI_FORMAT c_colorFormat = DXGI_FORMAT_R8G8B8A8_UNORM;
    constexpr XG_FORMAT   c_colorXGFormat = XG_FORMAT_R8G8B8A8_UNORM;
    constexpr DXGI_FORMAT c_depthFormat = DXGI_FORMAT_D32_FLOAT;
    constexpr XG_FORMAT   c_depthXGFormat = XG_FORMAT_D32_FLOAT;

    constexpr int       c_esramPageCount = 512;
    constexpr int       c_pageSize = 64 * 1024;

    constexpr int       c_esramTexWidth = 4096;
    constexpr int       c_esramTexHeight = 2048;
...
void Sample::Render()
{
    ...
    auto const rtvDescriptor = m_rtvPile->GetCpuHandle(RTV_Main);
    auto const dsvDescriptor = m_dsvPile->GetCpuHandle(DSV_Main);
...

    {
        ...
        if (m_showOverlay)
        {
            ...
            const D3D12_VIEWPORT viewport = { 0.0f, 0.0f, float(c_esramTexWidth), float(c_esramTexHeight), D3D12_MIN_DEPTH, D3D12_MAX_DEPTH };
            auto const scissorRect = CD3DX12_RECT(0, 0, c_esramTexWidth, c_esramTexHeight);
    ...
    // Show the new frame.
    {
        ScopedPixEvent scopedEvent(commandList, PIX_COLOR_DEFAULT, L"Present");
...
void Sample::CreateDeviceDependentResources()
{
    ...
    RenderTargetState rtState(c_colorFormat, c_depthFormat, 2);
...
void Sample::CreateWindowSizeDependentResources()
{
    ...
    UINT32 numEsramPages = SupportsESRAM() ? UINT32(c_esramPageCount) : 0u;
...
void Sample::UpdateResourceMappings()
{
    ...
    if (m_layout == nullptr)
    {
        ...
        DX::ThrowIfFailed(m_layoutEngine.CreateMemoryLayout(L"Default Layout", size_t(totalPageCount * c_pageSize), 0, m_layout.ReleaseAndGetAddressOf()));
    }
    else
    {
        m_deviceResources->WaitForGpu();
    ...
    case EMS_Metadata:
        ...
        if (m_metadataEnabled)
        {
            auto createMetadataMapping = [&](const wchar_t* name, int totalPageCount, MetadataDesc& desc, XGMemoryLayoutMapping& mapping)
            {
                ...
                DX::ThrowIfFailed(m_layout->MapRunLengthArray(&mapping, RLETokens, UINT32(count), UINT32(totalPageCount), UINT32(XG_ALL_REMAINING_PAGES)));
            };
    ...
    if (SupportsESRAM())
    {
        ...
        m_layout->MapSimple(&esramOverlayMapping, UINT32(m_esramOverlayPageCount), UINT32(c_esramPageCount));
...
bool Sample::SupportsESRAM() const
{
    assert(m_deviceResources->GetD3DDevice() != nullptr);
    D3D12XBOX_GPU_HARDWARE_CONFIGURATION gpuHardwareConfiguration = {};
    m_deviceResources->GetD3DDevice()->GetGpuHardwareConfigurationX(&gpuHardwareConfiguration);
    return (gpuHardwareConfiguration.HardwareVersion <= D3D12XBOX_HARDWARE_VERSION_XBOX_ONE_S);
}
#pragma endregion
