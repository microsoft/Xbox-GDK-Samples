...
void DeviceResources::CreateDeviceResources(D3D12XBOX_CREATE_DEVICE_FLAGS createDeviceFlags)
{
    ...
    for (UINT n = 0; n < m_backBufferCount; n++)
    {
        ...
        for (unsigned int t = 0; t < MAX_THREADS_BVH_BUILD; ++t)
        {
            ThrowIfFailed(m_d3dDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_GRAPHICS_PPV_ARGS(m_commandAllocatorsBVH[n][t].ReleaseAndGetAddressOf())));

            swprintf_s(name, L"BVH build %u %u", n, t);
            m_commandAllocatorsBVH[n][t]->SetName(name);
        }
    }
    ...
    for (unsigned int t = 0; t < MAX_THREADS_BVH_BUILD; ++t)
    {
        ThrowIfFailed(m_d3dDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_commandAllocatorsBVH[0][t].Get(), nullptr, IID_GRAPHICS_PPV_ARGS(m_commandListsBVH[t].ReleaseAndGetAddressOf())));
        ThrowIfFailed(m_commandListsBVH[t]->Close());
    }
...
// Prepare the BVH command lists
void DeviceResources::PrepareBVHCommandLists()
{
    for (unsigned int t = 0; t < MAX_THREADS_BVH_BUILD; ++t)
    {
        ThrowIfFailed(m_commandAllocatorsBVH[m_backBufferIndex][t]->Reset());
        ThrowIfFailed(m_commandListsBVH[t]->Reset(m_commandAllocatorsBVH[m_backBufferIndex][t].Get(), nullptr));
    }
}

// Execute the BVH building command lists
void DeviceResources::ExecuteBVHCommandLists()
{
    ID3D12CommandList* ppCommandLists[MAX_THREADS_BVH_BUILD] = {};
    for (unsigned int t = 0; t < MAX_THREADS_BVH_BUILD; ++t)
    {
        ThrowIfFailed(m_commandListsBVH[t]->Close());
        ppCommandLists[t] = m_commandListsBVH[t].Get();
    }
    m_commandQueue->ExecuteCommandLists(MAX_THREADS_BVH_BUILD, ppCommandLists);
}
