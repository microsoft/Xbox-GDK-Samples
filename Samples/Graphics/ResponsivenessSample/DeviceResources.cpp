...
//
// Modified for exposing interval controls and frame stats.
//
...
DeviceResources::DeviceResources(
    ...
    unsigned int flags) noexcept(false) :
        ...
        m_pastFramePipelineTokens{},
        m_pastFramePipelineTokenIndex(0),
        ...
        m_backBufferCountLimiter(backBufferCount),
        m_clearColor{},
        m_window(nullptr),
...
void DeviceResources::WaitForOrigin()
{
    ...
    m_pastFramePipelineTokens[m_pastFramePipelineTokenIndex] = m_framePipelineToken;
    m_pastFramePipelineTokenIndex = (m_pastFramePipelineTokenIndex + 1) % MAX_FRAME_STATISTICS;
}
...
void DeviceResources::SetFrameIntervalX(UINT LengthInMicroseconds, UINT PeriodInIntervals, UINT IntervalOffsetInMicroseconds, D3D12XBOX_FRAME_INTERVAL_FLAGS flags)
{
    // First, retrieve the underlying DXGI device from the D3D device.
    ComPtr<IDXGIDevice1> dxgiDevice;
    ThrowIfFailed(m_d3dDevice.As(&dxgiDevice));
    ...
    ThrowIfFailed(m_d3dDevice->SetFrameIntervalX(
        ...
        PeriodInIntervals /* Allow N frames of latency */,
        flags));
...
HRESULT DeviceResources::GetFrameStatisticsX(
    UINT previousFrame,
    D3D12XBOX_FRAME_STATISTICS_TYPE TypeSet,
    UINT* pCount,
    D3D12XBOX_FRAME_STATISTICS* pStatistics)
{
    if (previousFrame >= MAX_FRAME_STATISTICS) { return E_INVALIDARG; }

    UINT index = (m_pastFramePipelineTokenIndex + MAX_FRAME_STATISTICS - previousFrame) % MAX_FRAME_STATISTICS;
    D3D12XBOX_FRAME_PIPELINE_TOKEN Token = m_pastFramePipelineTokens[index];

    if (Token == 0) { return E_INVALIDARG; }

    return m_d3dDevice->GetFrameStatisticsX(
        Token,
        TypeSet,
        pCount,
        pStatistics);
