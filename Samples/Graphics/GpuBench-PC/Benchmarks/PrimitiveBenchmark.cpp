...
//#define ENABLE_PRE_VS_POST_VS_MINIMUM_PRIMITIVE_THROUGHPUT 1
...
public:
    ...
    void Initialize(ID3D12Device* device) override
    {
        for (const auto& topologyParams : PrimitiveTest::m_allTopologyParams)
        {
            for (auto splitWorkIntoDraws : {false, true,})
            {
                for (auto splitInstanceIntoPrims : {false, true,})
                {
                    ...
                    AddTest(new PrimitiveTest(topologyParams, splitWorkIntoDraws, splitInstanceIntoPrims, D3D12_PIPELINE_STATE_FLAG_NONE));
#endif
                }
            }
        }
        ...
        {
            AddCounter("pda__cycles_elapsed.sum");
            AddCounter("pda__input_prims.sum");
            AddCounter("pda__input_prims.sum.per_cycle_elapsed");
            AddCounter("pda__input_prims.sum.pct_of_peak_sustained_elapsed");

        #ifdef ENABLE_PRE_VS_POST_VS_MINIMUM_PRIMITIVE_THROUGHPUT
            AddCounter("vpc__cycles_elapsed.sum");
            AddCounter("vpc__input_prims.sum");
            AddCounter("vpc__input_prims.sum.per_cycle_elapsed");
            AddCounter("vpc__input_prims.sum.pct_of_peak_sustained_elapsed");
        #endif
        }
        else if (gpuProps.IsSupportedAmdGpu())
        {
            AddCounter("VsGsPrimsIn");
...
private:
    ...
    public:
        ...
        static void PutHeader(Report* report)
        {
            ...
            report->AddColumn(L"Max Prims/clock", L"", 10, 2);
        ...
        void PutReport(Report* report) const override
        {
            ...
            auto primsThroughputPct = 100.0f * primsPerClock / primsTheoreticalMax;
#else // defined(_GAMING_DESKTOP)
        auto primsGPU = 0U;
        auto primsPerClock = 0.0;
        auto primsThroughputPct = 0.0;
        auto primsTheoreticalMax = 0U;

        if (gpuProperties.IsSupportedNvidiaGpu())
        {
        #ifdef ENABLE_PRE_VS_POST_VS_MINIMUM_PRIMITIVE_THROUGHPUT
            primsGPU = static_cast<uint32_t>(GetCounterValue("pda__input_prims.sum"))
                     + static_cast<uint32_t>(GetCounterValue("vpc__input_prims.sum"));

            primsPerClock = std::min(
                GetCounterValue("pda__input_prims.sum.per_cycle_elapsed"),
                GetCounterValue("vpc__input_prims.sum.per_cycle_elapsed")
            );

            primsThroughputPct = std::min(
                GetCounterValue("pda__input_prims.sum.pct_of_peak_sustained_elapsed"),
                GetCounterValue("vpc__input_prims.sum.pct_of_peak_sustained_elapsed")
            );
        #else
            primsGPU = static_cast<uint32_t>(GetCounterValue("pda__input_prims.sum"));
            primsPerClock = GetCounterValue("pda__input_prims.sum.per_cycle_elapsed");
            primsThroughputPct = GetCounterValue("pda__input_prims.sum.pct_of_peak_sustained_elapsed");
        #endif
            primsTheoreticalMax = static_cast<uint32_t>(primsPerClock * 100.0 / primsThroughputPct);
        }
        else if (gpuProperties.IsSupportedAmdGpu())
        {
            auto primsF64 = GetCounterValue("VsGsPrimsIn");
            auto cyclesF64 = GetCounterValue("GPUBusyCycles");
            primsGPU = static_cast<uint32_t>(primsF64);
            primsPerClock = primsF64 / cyclesF64;
        }
#endif
            ...
            report->AddRowData(primsThroughputPct);
            report->AddRowData(primsTheoreticalMax);
