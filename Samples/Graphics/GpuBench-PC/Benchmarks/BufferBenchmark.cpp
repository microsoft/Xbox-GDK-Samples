    ...
    void Initialize(ID3D12Device* device) override
    {
        ...
        for (auto& memOpParams : BufferTest::m_allMemOpParams)
        {
            for (auto& formatParams : BufferTest::m_allFormatParams)
            {
                ...
                if ((BufferTest::MemOpType::OP_VMEM_WRITE == memOpParams.m_type))
                {
                    continue;
                }
        ...
        {
            // The below counters can be used to count "uniform" buffer read instruction from Buffer<T>, their IPC and Throughput
            // NOTE: LSU includes other instructions (LDS read/write) but since benchmark shader isn't using other instructions, we can use this metric

            AddCounter("sm__inst_executed_pipe_lsu.sum");
            AddCounter("sm__inst_executed_pipe_lsu.sum.per_cycle_elapsed");
            AddCounter("sm__inst_executed_pipe_lsu.sum.pct_of_peak_sustained_elapsed");
        }
        else if (gpuProps.IsSupportedAmdGpu())
        {
            #define AmdGpu_SQPrefix "(SQWGP\\d+_SQ|SQ\\d+)_PERF_SEL_"
            AddCounter(AmdGpu_SQPrefix "INSTS_SMEM");
        }
...
private:
    ...
    public:
            ...
            m_descriptorRawUavGpu{}
        {
            ...
            // are made by the shader. The Dispatch will have a m_sizeY whose purpose is to repeat operations enough times
        ...
        void Initialize(ID3D12Device* device) override
        {
            ...
            else
            {
                ...
                // performance on Xbox One, even though no writes are done to it.
                descBuf.Flags |= D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;
...
#ifdef _GAMING_DESKTOP
            D3D12_DESCRIPTOR_HEAP_DESC descHeapCpuReadable =
            {
                D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV,             // D3D12_DESCRIPTOR_HEAP_TYPE Type;
                4U,                                                 // UINT NumDescriptors;
            ...
            DX::ThrowIfFailed(device->CreateDescriptorHeap(&descHeapCpuReadable, IID_GRAPHICS_PPV_ARGS(m_descriptorHeapCpuReadable.ReleaseAndGetAddressOf())));
            SET_NAME_TO_SELF(m_descriptorHeapCpuReadable);
#endif
...
#else
                NullDescriptorTypedBufferSrv(descriptorSrvCpu, device);
                NullDescriptorRawBufferSrv(descriptorRawSrvCpu, device);
#endif
            }
            else
            {
            ...
            m_descriptorUavCpu = CD3DX12_CPU_DESCRIPTOR_HANDLE(m_descriptorHeap->GetCPUDescriptorHandleForHeapStart(), 
                ...
                device->GetDescriptorHandleIncrementSize(descHeap.Type));
#ifdef _GAMING_DESKTOP
            m_descriptorUavCpuReadable = CD3DX12_CPU_DESCRIPTOR_HANDLE(m_descriptorHeapCpuReadable->GetCPUDescriptorHandleForHeapStart(),
                descriptorIndex,
                device->GetDescriptorHandleIncrementSize(descHeapCpuReadable.Type));
#endif
...
#else
                NullDescriptorTypedBufferUav(m_descriptorUavCpu, device);
                NullDescriptorTypedBufferUav(m_descriptorUavCpuReadable, device);
                NullDescriptorRawBufferUav(descriptorRawUavCpu, device);
#endif
            }
            else
            {
...
#ifdef _GAMING_DESKTOP
                device->CreateUnorderedAccessView(m_bufWrite.Get(), nullptr, &descUav, m_descriptorUavCpuReadable);
#endif
...
#ifdef _GAMING_DESKTOP
            m_descriptorHeapCpuReadable.Reset();
#endif
        }
        ...
        void PutReport(Report* report) const override
        {
            ...
            auto threadPerWave = m_threadPerWave;
#else
            auto threadPerWave = GetCsWaveSize();
#endif
            auto timeMs = GetElapsedTimeMs();

            auto memPerInstruction = m_memOpParams.m_scalar ? 1 : threadPerWave;
            auto memApi = m_sizeX * m_sizeY * m_sizeZ * (m_memOpParams.m_opsExpected + m_memOpParams.m_opsExtra) / (threadPerWave / memPerInstruction);
...
#else // defined(_GAMING_DESKTOP)
            auto memGpu = 0ULL;
            auto memPerClock = 0.0;
            auto memThroughput = 0.0;
            if (m_memOpParams.m_scalar)
            {
                auto memGpuF64 = 0.0;
                if (gpuProperties.IsSupportedNvidiaGpu())
                {
                    memGpuF64 = GetCounterValue("sm__inst_executed_pipe_lsu.sum");
                    memPerClock = GetCounterValue("sm__inst_executed_pipe_lsu.sum.per_cycle_elapsed");
                    memThroughput = GetCounterValue("sm__inst_executed_pipe_lsu.sum.pct_of_peak_sustained_elapsed");
                }
                else if (gpuProperties.IsSupportedAmdGpu())
                {
                    memGpuF64 = GetCounterValue(AmdGpu_SQPrefix "INSTS_SMEM");
                    memPerClock = memGpuF64 / GetCounterValue("GPUBusyCycles");
                }
                memGpu = static_cast<uint64_t>(memGpuF64);
            }
            else
            {
                GetVMemInstructions(threadPerWave, &memGpu, &memPerClock, &memThroughput);
            }
#endif
            ...
            auto memThroughput = 100.0f * memPerClock / memPerClockIdeal;
            ...
            report->AddRowData(memThroughput);
...
#else
                commandList->ClearUnorderedAccessViewUint(m_descriptorUavGpu, m_descriptorUavCpuReadable, m_bufWrite.Get(), clearValue, 0U, nullptr);
#endif
...
#ifdef _GAMING_DESKTOP
        ComPtr<ID3D12DescriptorHeap>        m_descriptorHeapCpuReadable;
#endif
...
#ifdef _GAMING_DESKTOP
        D3D12_CPU_DESCRIPTOR_HANDLE         m_descriptorUavCpuReadable;
#endif
