...
void DeviceResources::CreateDeviceResources()
{
...
#if defined(_DEBUG) && !defined(ENABLE_GPU_COUNTERS)
    ...
    GetAdapter(m_dxgiAdapter.GetAddressOf());
...
#if !defined(NDEBUG) && !defined(ENABLE_GPU_COUNTERS)
...
{
    // Schedule a Signal command in the queue to mark the point when the Command Allocator becomes no longer in use and therefore can be re-used.
    ThrowIfFailed(m_commandQueue->Signal(m_fence.Get(), m_fenceValue));
    // Track the current fence value for Command Allocator.
    m_commandAllocatorsFence[m_backBufferIndex] = m_fenceValue;

    // Update the back buffer index.
    m_backBufferIndex = m_swapChain->GetCurrentBackBufferIndex();

    // Check if Command Allocator corresponding to the new back buffer is not yet finished. Wait for it.
    if (m_fence->GetCompletedValue() < m_commandAllocatorsFence[m_backBufferIndex])
    {
        ThrowIfFailed(m_fence->SetEventOnCompletion(m_commandAllocatorsFence[m_backBufferIndex], m_fenceEvent.Get()));
        std::ignore = WaitForSingleObjectEx(m_fenceEvent.Get(), INFINITE, FALSE);
    }
