...
Interpolants main(uint VertexId : SV_VertexId)
{
    ...
    const float freq = 0.125f;  // Slow down the period

    float t = (sin(Time) + 1.0f) * 0.5f + Time; // Reparameterize time to make movement more interesting

    float v = 2.0f * 3.1415926f * freq * t;
    float c = cos(v);
    float s = sin(v);
