...
#endif
{
    ...
    uavDescriptorHeapDesc.NumDescriptors = m_backBufferCount;
    uavDescriptorHeapDesc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
    ThrowIfFailed(m_d3dDevice->CreateDescriptorHeap(&uavDescriptorHeapDesc, IID_GRAPHICS_PPV_ARGS(m_uavDescriptorHeap.ReleaseAndGetAddressOf())));
    m_uavDescriptorHeap->SetName(L"DeviceResources");
    m_uavDescriptorSize = m_d3dDevice->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
...
void DeviceResources::CreateWindowSizeDependentResources()
{
    ...
    // For Xbox One, a title needs to specify the auto tone mapping flag, but not on Scarlett
    ...
    for (UINT n = 0; n < m_backBufferCount; n++)
    {
        ThrowIfFailed(m_d3dDevice->CreateCommittedResource(
            ...
            IID_GRAPHICS_PPV_ARGS(m_renderTargets[n].ReleaseAndGetAddressOf())));

        wchar_t name[25] = {};
        swprintf_s(name, L"SwapBuffer %u", n);
        ...
        CD3DX12_CPU_DESCRIPTOR_HANDLE uavDescriptor(
            m_uavDescriptorHeap->GetCPUDescriptorHandleForHeapStart(),
            static_cast<INT>(n), m_uavDescriptorSize);

        m_d3dDevice->CreateUnorderedAccessView(m_renderTargets[n].Get(), nullptr, nullptr, uavDescriptor);
    }
...
void DeviceResources::Present(D3D12_RESOURCE_STATES beforeState, _In_opt_ const D3D12XBOX_PRESENT_PARAMETERS* params)
{
    ...
    {
#if defined(_GAMING_XBOX_SCARLETT) && (_GXDK_VER >= 0x4A6117BE /* GDK Edition 210400 */)
        // For Xbox Series X|S, we recommend presenting linear P3-D65 colors and let the display hardware convert to
        // Rec.2020 and apply the ST.2084 gamma curve. The HDRPrecision sample demonstrates all the different options
        planeParameters.ColorSpace = DXGI_COLOR_SPACE_RGB_FULL_G10_NONE_D65P3;
#else
        // For Xbox One, we tell the driver that the swap chain contains non-linear color values in Rec.2020 color space
        // with the ST.2084 gamma curve applied. A shader will convert from linear Rec.709 to HDR10
