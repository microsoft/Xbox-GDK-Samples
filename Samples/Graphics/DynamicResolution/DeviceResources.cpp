...
//
// NOTE: This is a customized version of DeviceResources.cpp that includes support for dynamic resolution.
//
...
DeviceResources::DeviceResources(
    ...
    unsigned int flags) noexcept(false) :
        ...
        m_options(flags),
        m_presentationThreshold(0.0f),
        m_sceneRenderSize{0, 0, 1920, 1080}
{
...
void DeviceResources::CreateDeviceResources()
{
    ...
    rtvDescriptorHeapDesc.NumDescriptors = m_backBufferCount * 2;   // separate RTs for scene and UI
    ...
    if (m_options & (c_Enable4K_UHD | c_EnableQHD))
    {
        ...
        m_sceneRenderSize = m_outputSize;
    }
...
void DeviceResources::CreateWindowSizeDependentResources()
{
    ...
    for (UINT n = 0; n < m_backBufferCount; n++)
    {
        ...
        wchar_t name[25] = {};
        swprintf_s(name, L"Scene render target %u", n);
        ...
        ThrowIfFailed(m_d3dDevice->CreateCommittedResource(
            ...
            D3D12_RESOURCE_STATE_PRESENT,
            &swapChainOptimizedClearValue,
            IID_GRAPHICS_PPV_ARGS(m_renderTargetsUI[n].GetAddressOf())));

        swprintf_s(name, L"UI render target %u", n);
        m_renderTargetsUI[n]->SetName(name);

        const CD3DX12_CPU_DESCRIPTOR_HANDLE rtvDescriptorGameUI(
            m_rtvDescriptorHeap->GetCPUDescriptorHandleForHeapStart(),
            static_cast<INT>(m_backBufferCount + n), m_rtvDescriptorSize);
        m_d3dDevice->CreateRenderTargetView(m_renderTargetsUI[n].Get(), &rtvDesc, rtvDescriptorGameUI);
    }
...
void DeviceResources::Prepare(D3D12_RESOURCE_STATES beforeState, D3D12_RESOURCE_STATES afterState)
{
    ...
    if (beforeState != afterState)
    {
        ...
        const D3D12_RESOURCE_BARRIER barriers[2] =
        {
            ...
            CD3DX12_RESOURCE_BARRIER::Transition(m_renderTargetsUI[m_backBufferIndex].Get(), beforeState, afterState),
        };
...
void DeviceResources::Present(uint32_t currentWidth, uint32_t currentHeight, D3D12_RESOURCE_STATES beforeState,
    _In_opt_ const D3D12XBOX_PRESENT_PARAMETERS* params)
{
    if (beforeState != D3D12_RESOURCE_STATE_PRESENT)
    {
        ...
        const D3D12_RESOURCE_BARRIER barriers[2] =
        {
            ...
            CD3DX12_RESOURCE_BARRIER::Transition(m_renderTargetsUI[m_backBufferIndex].Get(), beforeState, D3D12_RESOURCE_STATE_PRESENT),
        };
        m_commandList->ResourceBarrier(static_cast<UINT>(std::size(barriers)), barriers);
    }
    ...
    planeParameters[0].pSrcViewRects = &m_sceneRenderSize;
    planeParameters[0].ppResources = m_renderTargets[m_backBufferIndex].GetAddressOf();
    planeParameters[0].ColorSpace = (m_options & c_EnableHDR) ? DXGI_COLOR_SPACE_RGB_FULL_G2084_NONE_P2020 : DXGI_COLOR_SPACE_RGB_FULL_G22_NONE_P709;

    planeParameters[1].pSrcViewRects = &m_outputSize;
    planeParameters[1].ppResources = m_renderTargetsUI[m_backBufferIndex].GetAddressOf();
    planeParameters[1].ColorSpace = DXGI_COLOR_SPACE_RGB_FULL_G22_NONE_P709;

    D3D12XBOX_PRESENT_PARAMETERS presentParameters = (params) ? *params : D3D12XBOX_PRESENT_PARAMETERS{};
    presentParameters.ImmediateThresholdPercent = m_presentationThreshold;

    // The code block below defines the extended present needed for the sample to support enabling
    // Title Performance Overlay. For more info on this, refer to the GDK docs for the section titled
    // "Title Performance Overlay". This feature is only supported on Scarlett. 
#ifdef _GAMING_XBOX_SCARLETT
    D3D12XBOX_PRESENT_DESC_TITLE_PERFORMANCE_OVERLAY presentDescTitleOverlay = {};
    presentDescTitleOverlay.Flags = D3D12XBOX_PRESENT_DESC_TITLE_PERFORMANCE_OVERLAY_FLAGS_NONE;
    presentDescTitleOverlay.RenderResolutionWidth = static_cast<uint16_t>(currentWidth);
    presentDescTitleOverlay.RenderResolutionHeight = static_cast<uint16_t>(currentHeight);
    presentDescTitleOverlay.MaxResolutionWidth = static_cast<uint16_t>(m_outputSize.right);
    presentDescTitleOverlay.MaxResolutionHeight = static_cast<uint16_t>(m_outputSize.bottom);

    D3D12XBOX_PRESENT_DESC presentExtendedDesc = {};
    presentExtendedDesc.Type = D3D12XBOX_PRESENT_DESC_TYPE_TITLE_PERFORMANCE_OVERLAY;
    presentExtendedDesc.TitlePerfOverlay = presentDescTitleOverlay;

    presentParameters.ExtendedDescCount = 1;
    presentParameters.pExtendedDescs = &presentExtendedDesc;
#else
    std::ignore = currentWidth;
    std::ignore = currentHeight;
#endif

    ThrowIfFailed(m_commandQueue->PresentX(2, planeParameters, &presentParameters));
