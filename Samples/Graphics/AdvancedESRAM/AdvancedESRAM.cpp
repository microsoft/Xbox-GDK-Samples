...
namespace
{
    ...
    const DirectX::XMVECTORF32 s_visualizeColors[] = 
    {
...
void Sample::Update(DX::StepTimer const& timer)
{
    ...
    if (pad.IsConnected())
    {
        ...
        {
            IncrMod(m_outlineObjectIndex, m_scene.size());
            m_updateStats = true;
        }
        else if (m_gamePadButtons.leftShoulder == ButtonState::PRESSED)
        {
            DecrMod(m_outlineObjectIndex, m_scene.size());
            m_updateStats = true;
        }
        ...
        {
            IncrMod(m_visData.selectedIndex, (uint32_t)ST_Count);
            m_updateStats = true;
        }
        else if (m_gamePadButtons.dpadLeft == ButtonState::PRESSED)
        {
            DecrMod(m_visData.selectedIndex, (uint32_t)ST_Count);
            m_updateStats = true;
        }
        ...
        {
            uint32_t index = m_visData.selectedIndex;
...
void Sample::DrawHUD(ID3D12GraphicsCommandList* commandList)
{
    ...
    if (SupportsEsram())
    {
        ...
        m_smallFont->DrawString(m_hudBatch.get(), L"-- ESRAM Percentages --", textPos, textColor);
        textPos.y += m_smallFont->GetLineSpacing();
        ...
        for (size_t i = 0; i < ST_Count; ++i)
        {
            ...
            if (i == m_visData.selectedIndex)
                color = DirectX::XMVectorLerp(color, DirectX::g_XMOne, flashBlend);
        ...
        float charLen = XMVectorGetX(m_smallFont->MeasureString(L"_"));
...
void Sample::CreateWindowSizeDependentResources()
{
    ...
    XMUINT2 visSize = { uint32_t(m_displayWidth * 0.8f), uint32_t(m_displayHeight * 0.15f) };
    XMUINT2 visPad = { uint32_t(m_displayWidth * 0.1f), uint32_t(m_displayHeight * 0.17f) };

    // Initialize the constant visualization values.
    m_visData.bounds.x = visPad.x;
    m_visData.bounds.y = m_displayHeight - visPad.y - visSize.y;
    m_visData.bounds.z = m_displayWidth - visPad.x;
    ...
    for (size_t i = 0; i < ST_Count; ++i)
    {
        m_visData.textures[i].color = Color(s_visualizeColors[i]);
    }
    ...
    m_depthDesc = ATG::TransientDesc
    {
        CD3DX12_RESOURCE_DESC::Tex2D(
            ...
            1,                              // UINT SampleCount;
            0,                              // UINT Quality;
            D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL,       // D3D12_RESOURCE_FLAGS MiscFlags;
    ...
    m_outlineDesc.d3dDesc.Flags &= (~D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
    m_outlineDesc.d3dDesc.Flags |= D3D12XBOX_RESOURCE_FLAG_DENY_COMPRESSION_DATA;

    m_bloomDesc = m_colorDesc;
    m_bloomDesc.d3dDesc.Flags &= (~D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
...
{
    uint32_t pages = uint32_t(float(desc.GetPageCount()) * m_esramRatios[tex]);
...
void Sample::UpdateVisualizerRanges(const ResourceHandle(&resources)[ST_Count])
{
    ...
    for (size_t i = 0; i < std::size(resources); ++i)
    {
        ...
        m_visData.textures[i].pageRangeCount = (uint32_t)ranges.size();
