...
namespace
{
    ...
    inline uint32_t getCount(uint32_t token) { return token & ~c_esramTokenMask; }
}
...
namespace ATG
{
    ...
    { 
        ...
        void* address = XMemVirtualAlloc(nullptr, maxSizeBytes, MEM_64K_PAGES | MEM_RESERVE, XMEM_GRAPHICS | XMEM_MAPPABLE, PAGE_READWRITE | PAGE_WRITECOMBINE | PAGE_GRAPHICS_READWRITE);
    ...
    TransientResource TransientAllocator::Acquire(ID3D12GraphicsCommandList* list, const TransientDesc& desc, D3D12_RESOURCE_STATES initialState, uint32_t* tokens, uint32_t tokenCount, const wchar_t* name)
    {
        assert(list != nullptr);
    ...
    void TransientAllocator::AllocatePages(uint32_t pageCount, std::vector<PageRef>& pageRefs, uint32_t* tokens, uint32_t tokenCount)
    {
        ...
        if (m_useEsram && tokens != nullptr)
        {
            ...
            for (uint32_t i = 0; i < tokenCount && pageCount > 0 && remaining == 0; ++i)
            {
    ...
    void TransientAllocator::GeneratePageMappings(void* address, const std::vector<PageRef>& pageRefs)
    {
        ...
        for (const PageRef& page : pageRefs)
        {
            ...
            if (batchIndex == size_t(-1))
            {
                batchIndex = reinterpret_cast<size_t>(pool.pBatches) + pool.batchCount;

                D3D12XBOX_PAGE_MAPPING_BATCH& batch = *m_batches.emplace(m_batches.begin() + (ptrdiff_t)batchIndex);
                ...
                for (size_t i = poolIndex + 1; i < m_pools.size(); ++i)
                {
