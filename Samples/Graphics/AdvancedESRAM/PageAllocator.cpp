...
namespace ATG
{
    ...
    bool PageBlock::AllocateRange(uint32_t count, Range& outRange)
    {
        if (IsExhausted())
        {
            return false;
        ...
        uint32_t allocStart = range.start;
        uint32_t allocCount = std::min(count, range.count);
    ...
    void PageBlock::FreeRange(const Range& range)
    {
        ...
        if (!merged)
        {
            ...
            ranges.insert(ranges.begin() + (ptrdiff_t)i, range);
        }
    }
    ...
    void EsramMappingPolicy::InitBlock(ID3D12Device* device, void* address, uint32_t index, PageBlock& block)
    {
        UNREFERENCED_PARAMETER(index);

        // Map the virtual range to the entirety of ESRAM.
        UINT pages[s_blockPageCount];
        for (uint32_t i = 0; i < s_blockPageCount; ++i)
        {
            pages[i] = i;
        }
    ...
    void EsramMappingPolicy::DeinitBlock(ID3D12Device* device, void* address, uint32_t index, PageBlock& block)
    {
        UNREFERENCED_PARAMETER(index);

        // Release the page pool.
        device->UnregisterPagePoolX(block.pagePoolHandle);
        block.pagePoolHandle = nullptr;
        block.ranges.clear();
    ...
    bool DramMappingPolicy::CanExpand(uint32_t newBlockCount)
    {
        return s_blockSizeBytes * newBlockCount <= m_maxSizeBytes;
    }

    void DramMappingPolicy::InitBlock(ID3D12Device* device, void* address, uint32_t index, PageBlock& block)
    {
        // Allocate some memory to maintain list of allocated physical pages - needed to release the pages.
        uint32_t start = index * s_blockPageCount;

        uint32_t end = start + s_blockPageCount;
        ...
        BOOL success = XMemAllocatePhysicalPages(MEM_64K_PAGES, &count, pagePtr);
        if (!success)
        {
        ...
        void* blockAddress = XMemMapPhysicalPages(address, count, pagePtr);
    ...
    void DramMappingPolicy::DeinitBlock(ID3D12Device* device, void* address, uint32_t index, PageBlock& block)
    {
        UNREFERENCED_PARAMETER(index);
        ...
        XMemFreePhysicalPages(s_blockPageCount, pagePtr);
    }
}
