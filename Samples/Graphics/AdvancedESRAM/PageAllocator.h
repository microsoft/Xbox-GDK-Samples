...
namespace ATG
{
    ...
    public:
        ...
        bool IsFresh() const { return ranges.size() == 1ull && ranges[0].count == pagePoolCount; }
        // Attempts to allocate 'count' pages in a contiguous range.
        bool AllocateRange(uint32_t count, Range& outRange);
    ...
    public:
        ...
        //
        void Allocate(uint32_t count, std::vector<PageRef>& pageRefs, uint32_t* remaining = nullptr);
    ...
    private:
        ...
        bool Expand(uint32_t blockCount);
    ...
    private:
        ...
        uint32_t                m_highMark = 0;
        uint32_t                m_freePageCount = 0;
    ...
    {
    public:
        static constexpr uint32_t s_policyId = 0x0;
        static constexpr uint32_t s_blockPageCount = c_esramPageCount;
        static constexpr uint32_t s_blockSizeBytes = s_blockPageCount * c_pageSizeBytes;
    ...
    public:
        bool CanExpand(uint32_t newBlockCount) { return m_enabled && newBlockCount == 1u; }
        void InitBlock(ID3D12Device* device, void* address, uint32_t index, PageBlock& block);
        void DeinitBlock(ID3D12Device* device, void* address, uint32_t index, PageBlock& block);
    ...
    {
    public:
        static constexpr uint32_t s_policyId = 0x1;
        static constexpr uint32_t s_blockPageCount = c_dramBlockPageCount;
        static constexpr uint32_t s_blockSizeBytes = s_blockPageCount * c_pageSizeBytes;
    ...
    public:
        ...
        bool CanExpand(uint32_t newBlockCount);
        void InitBlock(ID3D12Device* device, void* address, uint32_t index, PageBlock& block);
        void DeinitBlock(ID3D12Device* device, void* address, uint32_t index, PageBlock& block);
