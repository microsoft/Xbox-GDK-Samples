...
namespace ATG
{
    ...
    void EsramVisualizeEffect::Process(ID3D12GraphicsCommandList* commandList)
    {
        ...
        // Dispatch
        uint32_t width = m_constants.bounds.z - m_constants.bounds.x;
        uint32_t height = m_constants.bounds.w - m_constants.bounds.y;

        uint32_t threadGroupX = (width - 1) / s_groupSize.x + 1;
        uint32_t threadGroupY = (height - 1) / s_groupSize.y + 1;
    ...
    void EsramVisualizeEffect::SetConstants(const Constants& constants)
    {
        ...
        for (uint32_t i = 0; i < _countof(Constants::textures); ++i)
        {
            ...
            for (uint32_t j = 0; j < tex.pageRangeCount; ++j)
            {
