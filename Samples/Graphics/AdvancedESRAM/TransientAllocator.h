...
namespace ATG
{
    ...
    public:
        ...
        //  Param name - (in_opt) name to specify as the resource's debug name.
        //
        TransientResource Acquire(ID3D12GraphicsCommandList* list, const TransientDesc& desc, D3D12_RESOURCE_STATES initialState, uint32_t* tokens, uint32_t tokenCount, const wchar_t* name = nullptr);
    ...
    private:
        void AllocatePages(uint32_t pageCount, std::vector<PageRef>& pageRefs, uint32_t* tokens, uint32_t tokenCount);
