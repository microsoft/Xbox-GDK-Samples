...
namespace
{
    ...
    float LinearToSRGB(float hdrSceneValue)
    {
        ...
        return Scale * std::powf(hdrSceneValue, InvGamma) - Bias;
    }
}
...
void Sample::RenderUI()
{
    ...
    m_fontBatch->Begin(d3dCommandList);
    m_textFont->DrawString(m_fontBatch.get(), L"HDR10 Sample for DirectX 12", fontPos, White, 0.0f, g_XMZero, 1.0f);
...
{
    // While a title is suspended, the console TV settings could have changed,
    // so we need to call the display APIs when resuming
    SetDisplayMode();
