...
DeviceResources::DeviceResources(
    DXGI_FORMAT sceneBackBufferFormat,
    ...
    unsigned int flags) noexcept(false) :
        ...
        m_screenViewport{},
        m_scissorRect{},
        m_sceneBackBufferFormat(sceneBackBufferFormat),
...
#endif
{
    ...
    rtvDescriptorHeapDesc.NumDescriptors = m_backBufferCount * 2;       // We present a scene and UI swap buffer
...
void DeviceResources::CreateWindowSizeDependentResources()
{
    ...
    CD3DX12_CLEAR_VALUE swapChainOptimizedClearValue(m_sceneBackBufferFormat, m_clearColor);
    ...
    for (UINT n = 0; n < m_backBufferCount; n++)
    {
        ThrowIfFailed(m_d3dDevice->CreateCommittedResource(
            ...
            IID_GRAPHICS_PPV_ARGS(m_sceneRenderTargets[n].GetAddressOf())));

        wchar_t name[25] = {};
        swprintf_s(name, L"Scene render target %u", n);
        m_sceneRenderTargets[n]->SetName(name);

        D3D12_RENDER_TARGET_VIEW_DESC rtvDesc = {};
        rtvDesc.Format = m_sceneBackBufferFormat;
        rtvDesc.ViewDimension = D3D12_RTV_DIMENSION_TEXTURE2D;

        const CD3DX12_CPU_DESCRIPTOR_HANDLE rtvDescriptor(
            m_rtvDescriptorHeap->GetCPUDescriptorHandleForHeapStart(),
            static_cast<INT>(n), m_rtvDescriptorSize);
        m_d3dDevice->CreateRenderTargetView(m_sceneRenderTargets[n].Get(), &rtvDesc, rtvDescriptor);
    }

    // UI render targets
    swapChainBufferDesc.Format = m_uiBackBufferFormat;
    swapChainOptimizedClearValue.Format = m_uiBackBufferFormat;
    for (UINT n = 0; n < m_backBufferCount; n++)
    {
        ThrowIfFailed(m_d3dDevice->CreateCommittedResource(
            &swapChainHeapProperties,
        ...
        D3D12_RENDER_TARGET_VIEW_DESC rtvDesc = {};
        rtvDesc.Format = m_uiBackBufferFormat;
        rtvDesc.ViewDimension = D3D12_RTV_DIMENSION_TEXTURE2D;

        auto const uiStartIndex = m_backBufferCount;
        const CD3DX12_CPU_DESCRIPTOR_HANDLE rtvDescriptor(
            m_rtvDescriptorHeap->GetCPUDescriptorHandleForHeapStart(),
            static_cast<INT>(uiStartIndex + n), m_rtvDescriptorSize);
...
void DeviceResources::Prepare(D3D12_RESOURCE_STATES beforeState, D3D12_RESOURCE_STATES afterState)
{
    ...
    if (beforeState != afterState)
    {
        ...
        {
            CD3DX12_RESOURCE_BARRIER::Transition(m_sceneRenderTargets[m_backBufferIndex].Get(), beforeState, afterState),
...
void DeviceResources::Present(D3D12_RESOURCE_STATES beforeState, _In_opt_ const D3D12XBOX_PRESENT_PARAMETERS* params)
{
    if (beforeState != D3D12_RESOURCE_STATE_PRESENT)
    {
        ...
        const D3D12_RESOURCE_BARRIER barriers[2] =
        {
            CD3DX12_RESOURCE_BARRIER::Transition(m_sceneRenderTargets[m_backBufferIndex].Get(), beforeState, D3D12_RESOURCE_STATE_PRESENT),
