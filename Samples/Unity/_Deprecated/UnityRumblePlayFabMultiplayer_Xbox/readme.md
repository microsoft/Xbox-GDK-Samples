...
**IMPORTANT:** To build and run on Xbox consoles you will need the Game
Core and Input System GXDK packages that are distributed by Unity. Make
sure to use the package versions that are required by your version of
Unity.\
\
![Graphical user interface, text Description automatically generated](./media/image3.png)

**IMPORTANT:** Once you have imported the playfab-party unitypackage
file, you must run the setup batch script located in the
"Assets\\PlayFabPartySDK\\Setup\\GameCore" folder. This batch script
will copy the correct versions of dlls that PlayFab Party has
dependencies on.

****

**\
IMPORTANT:** Once you have imported the playfab-multiplayer unitypackage
file, you must run the setup batch script located in the
...
**IMPORTANT:** You must also ensure that the PlayFabMultiplayer.dll is
configured for "GameCoreScarlett" and "GameCoreXboxOne".
...
Next, open the "Build Settings" menu and switch to either "Game Core --
Scarlett" or "Game Core -- Xbox One platforms". Then select "Build and
Run" from the bottom right.

![Graphical user interface Description automatically generated](./media/image7.png)
...
with the sample. Integrating a newer version of Unity's GameCore
package, or integrating a different version of the PlayFab, PlayFab
Party, PlayFab Multiplayer, or Unity plugin may expose incompatibilities
with APIs that may be deprecated, or have altered behavioral
characteristics.
