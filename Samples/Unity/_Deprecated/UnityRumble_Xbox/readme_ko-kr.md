...
#   Scarlett/XboxOne Unity GDK 럼블 샘플
...
이 샘플은 2021년 2월 Microsoft GDK 설치 및 GameCore용 Unity 버전
2020.3.12f1에 대해 개발 및 테스트되었습니다. 샘플은 샘플의 필수 SDK
종속성을 충족하기 위해 샘플의 Assets/ 폴더에 압축을 풀 수 있는 ZIP
파일(SDKs/ 폴더에 있음)에 제공된 스냅샷이 있는 여러 SDK에 따라 다릅니다.
샘플은 Assets/Sample/Scenes 폴더에 있는 \"SampleScene.unity\"라는 하나의
장면으로만 구성됩니다.
...
\... 샘플은 현재 선택한 빌드 폴더 아래에 느슨한 배포 폴더를 배치합니다.
예를 들어 \"Builds/Scarlett\"인 경우 배포 폴더는 다음과 같이
\"Builds/Scarlett/Loose\"(또는 \"Builds/XboxOne/Loose\")가 됩니다.

![A picture containing graphical user interface Description automatically generated](./media/image5.png)

\"MicrosoftGame.config\" 파일과 5개의 PNG 파일은 Scarlett 및 XboxOne
구성 설정 내에서 지정되며 GDK 응용 프로그램에 필요한 지원 파일을
제공합니다.

Unity가 빌드 프로세스를 완료하고 메타데이터 파일이 빌드 출력 폴더에
있으면 Scarlett/XboxOne 키트에 애플리케이션을 테스트용으로 로드"하는 "두
가지 방법이 있습니다.

1.  \"Gaming VS 2019 명령 프롬프트\"를 사용하여 샘플 루트 폴더에서
    \"xbapp deploy Builds\\Scarlett\\Loose\" 명령을 실행할 수 있습니다.

2.  \"Gaming VS 2019 명령 프롬프트\"를 사용하여 \"makepkg\" 명령을
    실행하여 \"xbapp install
    Builds\\\<path_to_package\>\\\<name_of_app_identity\>.XVC\" 후속
    명령을 사용하여 설치된 패키지 빌드를 만들 수 있습니다(GDK 참조
    \"makepkg\"에 대한 문서)

# 샘플 실행

개발 Scarlett 또는 XboxOne 키트에 앱을 성공적으로 빌드, 패키징 및 사이드
로드한 경우 \"UnityRumbleGDK.exe\"라는 이름의 앱 아이콘이 \"개발 홈 내\"
메뉴에 표시되어야 합니다. Xbox 관리자 또는 키트 자체를 사용하여 앱을
시작하면 시작 화면이 표시되고 샘플 전체 화면이 표시되어야 합니다.

성공적으로 실행하려면 타이틀이 구성 및 배포된 Xbox Live 샌드박스
\"XDKS.1\"(또는 자체 샌드박스)에서 Dev Kit가 실행되고 있어야 합니다.
Xbox 관리자 도구는 이 키트의 활성 샌드박스를 전환하는 데 도움이 될 수
있습니다. 샌드박스에 로그인할 수 있는 테스트 사용자 배치도 필요합니다.
