...
此示例是针对 2021 年 2 月 Microsoft GDK 安装和 Unity 版本 2020.3.12f1
开发和测试的。 该示例依赖于多个 SDK，所有这些 SDK 都在 ZIP 文件（位于
SDKs/ 文件夹中）中提供了快照，可以将其解压缩到示例的 Assets/
文件夹中以实现示例所需的 SDK 依赖项。 该示例仅包含一个名为
"SampleScene.unity" 的场景，位于 Assets/Sample/Scenes 文件夹中。
...
...此示例当前提供 "Builds" 文件夹，并可生成输出，进入名为
"Builds/PC/Loose" 的文件夹，如下所示：

![Graphical user interface Description automatically generated with medium confidence](./media/image5.png)

"MicrosoftGame.config" 文件和五个 PNG 文件位于 "GdkMetadata"
文件夹中，该文件夹中包含 GDK 应用程序所需的支持文件。

Unity
完成生成过程并将元数据文件复制到生成输出文件夹后，可通过两种方法将应用程序"旁加载"到
Windows 中：

1.  使用 "Gaming VS 2019 命令提示符"，可以从示例根文件夹执行 "wdapp
    register Builds\\PC\\Loose" 命令。

2.  使用 "Gaming VS 2019 命令提示符"，可以运行 "Builds\\package.bat"
    脚本，以生成使用后续命令"wdapp install
    Builds\\PC\\Package\\\<name_of_app_identity\> 安装的打包版本。MSIXVC

# 运行示例

如果已成功生成（可能为包），并将应用旁加载到 Windows 10 开发电脑上，则在
Windows 10 开始菜单中将显示名为 "UnityRumbleGDK" 的应用图标。
单击该应用图标可将示例启动到不可调整大小的 1920x1080 高清窗口中。

若要成功运行，必须在已配置和部署游戏的 Xbox Live 沙盒
"XDKS.1"（或你自己的沙盒）中运行电脑。 从"Gaming VS 2019
命令提示符"运行的名为 "XBLPCSandbox" 的 GDK 工具可以帮助切换活动沙盒。
你还需要一批可登录到沙盒的测试用户。
