...
struct SDKMESH_MATERIAL
{
    ...
    union
    {
        ...
        ID3D12Resource* pDiffuseTexture;
    };
    union
    {
        UINT64 Force64_2;           //Force the union to 64bits
        ID3D12Resource* pNormalTexture;
    };
    union
    {
        UINT64 Force64_3;           //Force the union to 64bits
        ID3D12Resource* pSpecularTexture;
    };
...
protected:
    HRESULT                         CreateVertexBuffer( SDKMeshLoad* pLoader,
                                                        SDKMESH_VERTEX_BUFFER_HEADER* pHeader, void* pVertices );

    HRESULT                         CreateIndexBuffer( SDKMeshLoad* pLoader, SDKMESH_INDEX_BUFFER_HEADER* pHeader,
                                                       void* pIndices );


    virtual HRESULT                 CreateFromFile( SDKMeshLoad* pLoader,
                                                    LPCTSTR szFileName );

    virtual HRESULT                 CreateFromMemory( SDKMeshLoad* pLoader,
                                                      BYTE* pData,
                                                      UINT DataBytes,
                                                      bool bCopyStatic );
    ...
    void SwapSDKMeshHeader( SDKMESH_HEADER * psdkMeshheader);

    void SwapVertexBufferHeaderArray( SDKMESH_VERTEX_BUFFER_HEADER * pVertexBufferHeaderArray , DWORD dwArrayLength );

    void SwapIndexBufferHeaderArray( SDKMESH_INDEX_BUFFER_HEADER * pIndexBufferHeaderArray , DWORD dwArrayLength );

    void SwapMeshArray( SDKMESH_MESH * pMeshArray , DWORD dwArrayLength );

    void SwapSubsetArray( SDKMESH_SUBSET * pSubsetArray , DWORD dwArrayLength );

    void SwapFrameArray( SDKMESH_FRAME * pFrameArray , DWORD dwArrayLength );

    void SwapMaterialArray( SDKMESH_MATERIAL * pMaterialArray , DWORD dwArrayLength );

    // Used to swap index and vertex buffers
    void SwapBuffer( DWORD * pBuffer , UINT64 uiNumBytes );
...
public:
    ...
    virtual HRESULT                 Create( SDKMeshLoad* pLoader, LPCTSTR szFileName );


    //Helpers (D3D11 specific)
    static D3D_PRIMITIVE_TOPOLOGY   GetPrimitiveType11( SDKMESH_PRIMITIVE_TYPE PrimType );
    DXGI_FORMAT                     GetIBFormat( UINT iMesh );
    ID3D12Resource* GetVB( UINT iMesh, UINT iVB );
    ID3D12Resource* GetIB( UINT iMesh );
    ...
    ID3D12Resource* GetVBAt( UINT iVB );
    ID3D12Resource* GetIBAt( UINT iIB );
