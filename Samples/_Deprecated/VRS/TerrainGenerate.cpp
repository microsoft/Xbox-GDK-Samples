...
void Terrain::GeneateTerrain(std::unique_ptr<DX::DeviceResources> &deviceResources)
{
    ...
    }
    {
        ...
        for (int i = 0; i < 8; ++i)
        {
            commandList->SetComputeRoot32BitConstant(ComputeTerrainRootConstants::CB1, 0, 0);
            commandList->Dispatch(m_sourceHeightMapSize / 8, m_sourceHeightMapSize / 8, 1);
            {
                // uav barriers
            ...
            commandList->SetComputeRoot32BitConstant(ComputeTerrainRootConstants::CB1, 1, 0);
            commandList->Dispatch(m_sourceHeightMapSize / 8, m_sourceHeightMapSize / 8, 1);
            {
                // uav barriers
                D3D12_RESOURCE_BARRIER barrier[2];
