...
{
    if (riid != __uuidof(IDStorageQueueX))
    {
        *ppvObject = nullptr;
...
/// \details The parameters must match the interface defined in the Microsoft GDK DirectStorage spec
//////////////////////////////////////////////////////////////////////////
...
/// \details Instantiation of IDStorageQueueX::EnqueueRequest function
//////////////////////////////////////////////////////////////////////////
void DirectStorageQueue::EnqueueRequest(const DSTORAGE_REQUEST* request)
{
...

    {
        ...
        if (request->File)   // Check the read location against the cached extents of the file
        {
            ...
            uint64_t fileSize = DirectStorageFactory::GetInstance()->GetFileSize(file->GetHandle());
...
/// \details Instantiation of IDStorageQueueX::EnqueueStatus function
//////////////////////////////////////////////////////////////////////////
void DirectStorageQueue::EnqueueStatus(IDStorageStatusArrayX* statusArray, UINT32 index)
{
    if (!m_entries)					// Queue has been closed
        return;
...
/// \details Instantiation of IDStorageQueueX::EnqueueSignal function
//////////////////////////////////////////////////////////////////////////
...
/// \details Instantiation of IDStorageQueueX::Submit function
//////////////////////////////////////////////////////////////////////////
...
/// \details Instantiation of IDStorageQueueX::CancelRequestsWithTag function
//////////////////////////////////////////////////////////////////////////
...
/// \details Instantiation of IDStorageQueueX::Close function
//////////////////////////////////////////////////////////////////////////
...
/// \details Instantiation of IDStorageQueueX::RetrieveErrorRecord function
//////////////////////////////////////////////////////////////////////////
...
/// \details Instantiation of IDStorageQueueX::Query function
...
DirectStorageQueue::SubmissionResult DirectStorageQueue::SubmitNextRequest()
{
    ...
    while (!SUCCEEDED(readError))
    {
        ...
        HANDLE file = (static_cast<DirectStorageFile*> (m_entries[m_nextRequestRead].request.File))->GetHandle();
...
DirectStorageQueue::DecompressionResult DirectStorageQueue::DecompressNextRequest()
{
    ...
    while ((curEntry != m_nextRequestRead) && !didDecompression)
    {
        if (m_entries[curEntry].IsReadyDecompression())
        {
            ...
            char* realSrcAddress = static_cast<char*> (m_entries[curEntry].stagingBuffer);
            ...
            size_t bcPackedSize = m_entries[curEntry].request.SourceSize;
            if (m_entries[curEntry].request.Options.ZlibDecompress)
            {
                bcPackedSize = LZInflate(zlibDestBuffer, m_entries[curEntry].request.DestinationSize, realSrcAddress, m_entries[curEntry].request.SourceSize);
            }
...
#else
            std::ignore = bcPackSrcBuffer;
            std::ignore = bcPackedSize;
            if (m_entries[curEntry].request.Options.BcpackMode != DSTORAGE_BCPACK_MODE_NONE)
            {
