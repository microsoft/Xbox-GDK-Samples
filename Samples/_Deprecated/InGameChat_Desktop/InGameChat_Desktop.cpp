...
void Sample::Initialize(HWND window, int width, int height)
{
    ...
    auto hr = XGameInviteRegisterForEvent(
        ...
        {
            // TODO: In 1906 use XSAPI to parse the uri
...
void Sample::LocalLobbyUpdate()
{
    ...
    {
        // [A] Join Chat Session
        if (m_keyboardButtons.IsKeyPressed(Keyboard::Keys::A) || m_gamePadButtons.a == GamePad::ButtonStateTracker::PRESSED)
        {
            m_state = SampleState::JoiningSession;
        ...
        // [X] Find Chat Session
        else if (m_keyboardButtons.IsKeyPressed(Keyboard::Keys::X) || m_gamePadButtons.x == GamePad::ButtonStateTracker::PRESSED)
        {
...
void Sample::JoinLobbyUpdate()
{
    ...
    if (m_keyboardButtons.IsKeyPressed(Keyboard::Keys::X) || m_gamePadButtons.x == GamePad::ButtonStateTracker::PRESSED)
    {
    ...
    else if (m_keyboardButtons.IsKeyPressed(Keyboard::Keys::B) || m_gamePadButtons.b == GamePad::ButtonStateTracker::PRESSED)
    {
        // Exit this screen
        m_state = SampleState::LocalLobby;
        m_ui->FindPanel<ATG::Overlay>(200)->Show();
...
void Sample::ChatLobbyUpdate()
{
    ...
    else if (m_keyboardButtons.IsKeyPressed(Keyboard::Keys::Y) || m_gamePadButtons.y == GamePad::ButtonStateTracker::PRESSED)
    {
        // Show Invite UI
        m_liveManager->InviteFriends(m_userManager->GetUsers().at(0)->UserHandle);
    }
    else if (m_keyboardButtons.IsKeyPressed(Keyboard::Keys::U) || m_gamePadButtons.rightShoulder == GamePad::ButtonStateTracker::PRESSED)
    {
        // Increase channel number
        ChangeChannel(1);
    }
    else if (m_keyboardButtons.IsKeyPressed(Keyboard::Keys::D) || m_gamePadButtons.leftShoulder == GamePad::ButtonStateTracker::PRESSED)
    {
...
void Sample::CreateWindowSizeDependentResources()
{
    ...
    console.right = console.left + 775;
