...
namespace ATG
{
    ...
    //  Desc: Sets up a new instance of the XAudio2 renderer for the specified device endpoint
    //--------------------------------------------------------------------------------------
    ...
    //--------------------------------------------------------------------------------------
    //  Name:   Stop
    //  Desc:   Stop playback, if XAudio2 renderer exists
    //--------------------------------------------------------------------------------------
    ...
    //--------------------------------------------------------------------------------------
    //  Name:   Shutdown
    //  Desc:   Stop and destroy XAudio2 engine and voices
    //--------------------------------------------------------------------------------------
    ...
    HRESULT XAudio2Manager::ConfigureWaveSource(const wchar_t* filename)
    {
        ...
        // Read the wave file
        WAVData waveData;
        hr = ATG::LoadWAVAudioFromFileEx(filename, m_waveFileData, waveData);
        if (FAILED(hr))
        {
            return hr;
        }
    ...
    //--------------------------------------------------------------------------------------
    HRESULT XAudio2Manager::ConfigureWaveSource(const uint8_t* wavData, size_t wavDataSize)
    {
        // Read the wave file
        WAVData waveData;
        HRESULT hr = ATG::LoadWAVAudioInMemoryEx(wavData, wavDataSize, waveData);
        if (FAILED(hr))
        {
            return hr;
        }
    ...
    //--------------------------------------------------------------------------------------
    HRESULT XAudio2Manager::ConfigureWaveSource(const WAVData& waveData)
    {
        // stop existing voice
        if (m_sourceVoice)
        {
        ...
        HRESULT hr = m_xAudio2->CreateSourceVoice(&m_sourceVoice, waveData.wfx, 0, XAUDIO2_DEFAULT_FREQ_RATIO, this);
        if (FAILED(hr))
        {
            return hr;
        }
        ...
        uint16_t wavChannels = waveData.wfx->nChannels;
    ...
    void __stdcall XAudio2Manager::OnVoiceProcessingPassStart(uint32_t) noexcept {}
