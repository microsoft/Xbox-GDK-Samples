...
void NavigationHint::SetClientDimensions(unsigned clientWidth, unsigned clientHeight)
{
    ...
    if (m_upIndicator)
    {
        ...
        upWidth = unsigned(upMeasurement.right - upMeasurement.left);
        upHeight = unsigned(upMeasurement.bottom - upMeasurement.top);
    }
    ...
    if(m_downIndicator)
    {
        ...
        downWidth = unsigned(downMeasurement.right - downMeasurement.left);
        downHeight = unsigned(downMeasurement.bottom - downMeasurement.top);
    }
    ...
    if (m_leftIndicator)
    {
        ...
        leftWidth = unsigned(leftMeasurement.right - leftMeasurement.left);
        leftHeight = unsigned(leftMeasurement.bottom - leftMeasurement.top);
    }
    ...
    if(m_rightIndicator)
    {
        ...
        rightWidth = unsigned(rightMeasurement.right - rightMeasurement.left);
        rightHeight = unsigned(rightMeasurement.bottom - rightMeasurement.top);
    }
    ...
    m_clientRect.left   = LONG(leftWidth);
    m_clientRect.right  = LONG(leftWidth + cw);
    m_clientRect.top    = LONG(upHeight);
    m_clientRect.bottom = LONG(upHeight + ch);
