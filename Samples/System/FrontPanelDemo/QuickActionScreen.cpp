...
QuickActionScreen::QuickActionScreen(FrontPanelManager *owner, XFrontPanelButton buttonId)
    : PanelScreen(owner)
    , m_myButton(buttonId)
...
void QuickActionScreen::RenderFrontPanel()
{
    ...
    // Draw the title text
    {
        titleFont.DrawStringFmt(fpDesc, unsigned(x), unsigned(y), L"Button %i Action", buttonIdx + 1);
        y += titleFont.GetLineSpacing();
    }

    // Draw the button graphic
    {
        wchar_t buttonGlyph = wchar_t(L'1' + buttonIdx);

        int bx = 2;
        int by = 16;

        symbolFont.DrawGlyph(fpDesc, unsigned(bx), unsigned(by), 0xE48C, buttonHasAssignment ? 0xF0u : 0x40u);
    ...
    // Draw the description text
    {
        ...
        {
            descriptionFont.DrawString(fpDesc, unsigned(x), unsigned(y), L"There is no action assigned to this\nbutton.");
        }
    }
...
void QuickActionScreen::SetLightState(bool on)
{
    ...
    XFrontPanelLight lights = m_owner->GetAssignedLights();
    
    if (on)
    {
        lights = lights | static_cast<XFrontPanelLight>(m_myButton);
    }
    else
    {
        lights = lights | static_cast<XFrontPanelLight>(~m_myButton);
