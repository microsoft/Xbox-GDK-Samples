    ...
    , m_descriptionFont(FontManager::Instance().LoadFont(L"assets\\Segoe_UI16.rasterfont"))
{
    ...
    m_nav.SetClientDimensions(clientWidth, unsigned(r.bottom - r.top));
}
...
void TopLevelScreen::RenderFrontPanel()
{
    ...
    // Draw the title text
    {
        m_titleFont.DrawString(fpDesc, unsigned(x), unsigned(y), m_titleText.c_str());
        y += m_titleFont.GetLineSpacing();
    }

    // Draw the label text
    {
        int x0 = int(fpDesc.width - m_nav.GetWidth());
        int y0 = 0;
        ...
        m_titleFont.DrawString(fpDesc, unsigned(x0), unsigned(y0), m_labelText.c_str());
    }


    // Draw the description text
    m_descriptionFont.DrawString(fpDesc, unsigned(x), unsigned(y), m_descriptionText.c_str());
