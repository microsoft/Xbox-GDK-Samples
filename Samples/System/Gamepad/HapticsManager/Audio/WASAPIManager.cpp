...
// WASAPIManager.cpp
//
// Wraps the underlying WASAPIRenderer in a simple class that can receive
...
namespace ATG
{
    ...
    //  Desc: Sets up a new instance of the WASAPI renderer for the specified device endpoint
    //--------------------------------------------------------------------------------------
    HRESULT WASAPIManager::InitializeDevice(wchar_t* endpoint, uint32_t locationCount, GUID* locations)
    {
        ...
        wfx->Format.nBlockAlign = static_cast<uint16_t>((wfx->Format.nChannels * wfx->Format.wBitsPerSample) / 8);
    ...
    HRESULT WASAPIManager::Play()
    {
        ...
        if (!m_playingSound || m_renderWorkThread == nullptr)
        {
                ...
                AUDCLNT_E_ENDPOINT_CREATE_FAILED == hr)
            {
                ...
                if (SUCCEEDED(hr) && m_renderWorkThread == nullptr)
                {
                    // Ensure the worker loop can run before creating the thread.
                    m_playingSound = true;

                    m_renderWorkThread = CreateThread(nullptr, 0, RenderWorkCallback, this, 0, nullptr);
                    if (m_renderWorkThread == nullptr)
                    {
            ...
            if (framesAvailable > 0)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        hr = m_audioRenderClient->GetBuffer(framesAvailable, &bufferData);
                        if (SUCCEEDED(hr))
                        {
                            hr = m_audioRenderClient->ReleaseBuffer(framesAvailable, AUDCLNT_BUFFERFLAGS_SILENT);
                        }
    ...
    //--------------------------------------------------------------------------------------
    HRESULT WASAPIManager::GetSample(uint32_t FramesAvailable)
    {
        HRESULT hr = S_OK;
        BYTE* Data;
    ...
    //  Desc:   Configures WAVE playback by filename
    //--------------------------------------------------------------------------------------
    HRESULT WASAPIManager::ConfigureWaveSource(const wchar_t* filename)
    {
        // Read the wave file
        WAVData waveData;
        HRESULT hr = LoadWAVAudioFromFileEx(filename, m_waveFileData, waveData);
        if (FAILED(hr))
            return hr;

        return ConfigureWaveSource(waveData);
    }

    //--------------------------------------------------------------------------------------
    //  Name:   ConfigureWaveSource
    //  Desc:   Configures WAVE playback by data buffer
    //--------------------------------------------------------------------------------------
    HRESULT WASAPIManager::ConfigureWaveSource(const uint8_t* wavData, size_t wavDataSize)
    {
        // Read the wave file
        WAVData waveData;
        HRESULT hr = LoadWAVAudioInMemoryEx(wavData, wavDataSize, waveData);
        if (FAILED(hr))
            return hr;
    ...
    //--------------------------------------------------------------------------------------
    //  Name:   ConfigureWaveSource
    //  Desc:   Configures WAVE playback by WAVData struct
    //--------------------------------------------------------------------------------------
    HRESULT WASAPIManager::ConfigureWaveSource(WAVData& waveData)
    {
        ...
        HRESULT hr = m_waveGenerator->GenerateSampleBuffer(&m_waveFileData[0], waveData.audioBytes, &wfx, m_framesPerPeriod, m_MixFormat);
        if (FAILED(hr))
        {
            return hr;
        }
