...
namespace
{
    std::unique_ptr<GameSaveSample::SampleGame> g_sample;
...
{
    OutputDebugStringW(L"Entered wWinMain in main.cpp\n");
    ...
    PAPPSTATE_REGISTRATION hPLM = {};
    {
        ...
        wcex.lpszClassName = u8"GameSaveWindowClass";
        wcex.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
        if (!RegisterClassExA(&wcex))
            return 1;

        // Create window
        HWND hwnd = CreateWindowExA(0, u8"GameSaveWindowClass", u8"GameSave", WS_OVERLAPPEDWINDOW,
...
void OnKeyUpMessage(GameSaveSample::SampleGame* game, UINT message, WPARAM wParam, LPARAM lParam)
{
    //CONSIDER: Based on how this is used, this should probably be OnKeyDown, as keydowns repeat...

    if (message != WM_KEYUP && message != WM_SYSKEYUP)
        return;

    DirectX::InputState& input = game->GetInputManager();
    ...
    case VK_SHIFT:
        ...
        vk = (((static_cast<UINT>(lParam) & 0x00ff0000) >> 16u) == 0x36) ? VK_RSHIFT : VK_LSHIFT;
#else
        vk = static_cast<int>(
...
{
    auto sample = reinterpret_cast<GameSaveSample::SampleGame*>(GetWindowLongPtr(hWnd, GWLP_USERDATA));
    ...
    case WM_KEYUP:
    case WM_SYSKEYUP:
        OnKeyUpMessage(sample, message, wParam, lParam);
        break;
