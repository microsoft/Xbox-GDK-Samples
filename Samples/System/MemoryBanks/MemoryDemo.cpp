...
namespace ATG
{
	...
	// Run through the entire binary tree validating all of the pointers
	...
	bool MemoryDemo::ShuffleRandomData(TestData *curNode)
	{
		...
		{       // we are building with /EHsc which means C++ try/catch will not catch OS exceptions like null pointer dereferences.
				// these types of exceptions require SEH to catch
			curNode->m_randomData = static_cast<uint32_t>(std::rand());
	...
	// Recursively create a binary tree using a stack based allocator pattern
	// The tree stores both the index to the start of the memory allocator block as well as a raw pointer
	// This is used in both TestFixedAddress and TestRandomAddress demonstrations
