...
namespace
{
    ...
    void DataBreakThread(DataBreakThreadParams& params)
    {
        ...
        if (SuspendThread(params.threadHandle) == (static_cast<DWORD> (-1)))          // The thread must be suspended to query its context without getting bogus data
            return;
        ...
        // debug register 4 and 5 are reserved, they will cause an invalid exception if accessed
        ...
        else
        {
            ...
            {
            case ADDRESS_SIZE::SIZE_1:                      // no alignment required
                break;
...
// FullName:  SetHardwareBreakPointForThread
...
//************************************
bool ATG::DataBreak::SetHardwareBreakPointForThread(HANDLE thread, void *address, DEBUG_REGISTER debugRegister, ADDRESS_OPERATION addressOperation, ADDRESS_SIZE addressSize)
{
    ...
    {
        auto dataBreakThread = new std::thread(DataBreakThread, std::ref(params));
...
// FullName:  ClearHardwareBreakPointForThread
// Returns:   bool indicating success or failure
...
//************************************
bool ATG::DataBreak::ClearHardwareBreakPointForThread(HANDLE thread, DEBUG_REGISTER debugRegister)
{
    DataBreakThreadParams params = {};
