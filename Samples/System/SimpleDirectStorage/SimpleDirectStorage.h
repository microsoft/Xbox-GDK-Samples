...
private:
    ...
    static constexpr uint64_t c_ONE_MEG = 1024 * 1024;
    static constexpr uint64_t c_NUM_MEGS_IN_GIG = 1024;
    static constexpr uint32_t c_dataBlockSize = c_NUM_MEGS_IN_GIG * c_ONE_MEG;		// The data file is created in block, this is due to the uint32_t vs. uint64_t max value and available memory
    static constexpr uint64_t c_dataFileSize = c_NUM_MEGS_IN_GIG * c_ONE_MEG * 5;	// this size file should give a decent amount of space for random loading. Couldn't hurt to increase this file size to increase the amount of seeking
    static constexpr uint64_t c_zipFileSize = c_ONE_MEG * 10;                       // these are the requested size, it is still rounded up to c_dataBlockSize

    static constexpr uint64_t c_realDataFileSize = (c_dataFileSize + (c_dataBlockSize - 1ULL)) & ~(c_dataBlockSize - 1ULL);		// round up to c_dataBlockSize
