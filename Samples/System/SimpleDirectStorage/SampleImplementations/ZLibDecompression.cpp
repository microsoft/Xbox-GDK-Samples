...
bool ZLibDecompression::RunSample(const std::wstring& fileName, uint32_t zipFileUncompressedSize)
{
    ...
    DX::ThrowIfFailed(factory->CreateStatusArray(1, u8"ZLibDecompression Status Array", __uuidof(IDStorageStatusArrayX), (void **)(status.ReleaseAndGetAddressOf())));
    ...
    request.DestinationSize = zipFileUncompressedSize;
    // Note: For compressed data FileOffset must be 16-byte aligned
    request.FileOffset = 0;
    request.SourceSize = compressedFileSize;
