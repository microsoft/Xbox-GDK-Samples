    ...
    public partial class MainWindow : Window
    {
        ...
        private void Window_Closing(object? sender, CancelEventArgs e)
        {
            ...
            AppendOutput("\nWait for the active operation to return before closing. " +
                "If it appears stalled, choose Cancel when available and wait for it to return. " +
                "If it remains unresponsive, end RemoteIterationToolsSample.exe in Task Manager as a last resort. " +
                "Force-closing does not undo completed transfers or confirm that the remote operation stopped.\n");
        }
        ...
        private async void Deploy_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (!TryReadAttributeMask(IncludeFileAttributesTextBox.Text, "Include File Attributes", out uint includeAttributes) ||
                !TryReadAttributeMask(ExcludeFileAttributesTextBox.Text, "Exclude File Attributes", out uint excludeAttributes))
                return;
            ...
            var operation = new OperationState("Deployment", ip, destination)
            {
                ...
                SearchOptions = new CopySearchOptions
                {
                    ...
                    IncludeFileAttributes = includeAttributes,
                    ExcludeFileAttributes = excludeAttributes
                }
            };
        ...
        private static bool TryReadAttributeMask(string text, string name, out uint value)
        {
            text = text.Trim();
            value = 0;
            ...
            System.Windows.MessageBox.Show($"{name} must be a decimal number between 0 and {uint.MaxValue}.", "Invalid attributes");
            return false;
        }
