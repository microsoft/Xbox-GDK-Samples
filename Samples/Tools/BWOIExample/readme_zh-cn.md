...
单个开发人员应该同时在计算机中安装有编译器工具集和所需的 SDK，以完成日常工作。 除标题和库以外，*Microsoft 游戏开发工具包* (GDK) 还提供了用于调试的 Visual Studio 集成、MSBuild 平台定义和分析工具。 但是，如果在进行日常生成工作时对标题和库可以使用&ldquo;xcopy-style&rdquo;部署，则将大大简化维护内部版本服务器的工作。 此示例演示了一种在未安装 Microsoft GDK 的情况下，使用 ** Gaming.\*.x64** 平台生成基于 MSBuild 项目的方法。 它还提供了使用 Windows 容器创建独立生成环境的选项，无需直接在主机上安装 Visual Studio。

# 软件设置

生成计算机通常安装有 Visual Studio 工具集和 Windows SDK，将其作为定期维护映像的一部分。 Azure DevOps&ldquo;[Microsoft 托管](https://docs.microsoft.com/en-us/azure/devops/pipelines/agents/hosted?view=azure-devops)&rdquo;就是如此，也是设置 [自托管 Windows 代理](https://docs.microsoft.com/en-us/azure/devops/pipelines/agents/v2-windows?view=azure-devops)或其他自定义生成计算机的特点。

对于构建 Microsoft GDK 项目，可以设置 [Visual Studio 2019](https://walbourn.github.io/visual-studio-2019/)（只能生成 v141 和 v142 平台工具集 VC++ 项目）或 [Visual Studio 2022 ](https://walbourn.github.io/visual-studio-2022/)（可以生成 v141、v142 和 v143 平台工具集 VC++ 项目）。 还可使用完整的 Visual Studio 安装或 [Visual Studio 生成工具](https://visualstudio.microsoft.com/downloads/#build-tools-for-visual-studio-2022)。 请确保安装以下组件：

**选项 1：完整的 Visual Studio 安装**

| 工作负荷 | 组件 ID（用于[命令行安装](https://docs.microsoft.com/en-us/visualstudio/install/use-command-line-parameters-to-install-visual-studio)） |
|---|---|
| 使用 C++ 进行游戏开发 | Microsoft.VisualStudio.Workload.NativeGame |
| 使用 C ++进行桌面开发<br /> *必需组件：*Windows 10 SDK （10.0.19041.0）或 Windows 11 SDK （10.0.22000.0）<br /><br /> *可选组件：*MSVC v141 - VS 2017 C++ x64/x86 生成工具 (v14.16)<br /> *仅在使用 VS 2019/MSBuild 16.0 或 VS 2022/MSBuild 17.0 生成 v141 平台工具集项目时必需*<br /><br />  *可选组件，仅 VS 2022：*MSVC v142 - VS 2019 C++ x64/x86 生成工具 (v14.29)<br /><br /> *仅在使用 VS 2022/MSBuild 17.0 生成 v142 平台工具集项目时必需* <br /><br /> *可选组件：*适用于 Windows (12.0.0 - x64/x86) 的 C++ Clang 工具 <br /> *仅在使用 Clang 工具集生成时必需* | Microsoft.VisualStudio.Workload.NativeDesktop<br /> Microsoft.VisualStudio.Component.Windows10SDK.19041<br />Microsoft.VisualStudio.Component.Windows11SDK.22000<br /><br /> *可选：*Microsoft.VisualStudio.Component.VC.v141.x86.x64<br /><br /> *可选，仅限 VS 2022：* Microsoft.VisualStudio.ComponentGroup.VC.Tools.142.x86.x64<br /><br /> *可选：* Microsoft.VisualStudio.ComponentGroup.NativeDesktop.Llvm.Clang |

**选项 2：Visual Studio 生成工具**

| 工作负荷 | 组件 ID（用于[命令行安装](https://docs.microsoft.com/en-us/visualstudio/install/use-command-line-parameters-to-install-visual-studio)） |
|---|---|
| C++ 生成工具<br /> *必需组件：*Windows 10 SDK （10.0.19041.0）或 Windows 11 SDK （10.0.22000.0）<br /><br /> *必需组件：*MSVC v142 - VS 2019 C++ x64/x86 生成工具（最新）<br /> - 或 - MSVC v143 - VS 2022 C++ x64/x86 生成工具（最新）<br /><br /> *可选组件：*MSVC v141 - VS 2017 C++ x64/x86 生成工具 (v14.16)<br /> *仅在使用 VS 2019/MSBuild 16.0 或 VS 2022/MSBuild 17.0 生成 v141 平台工具集项目时必需*<br /><br /> *可选组件，仅 VS 2022：*MSVC v142 - VS 2019 C++ x64/x86 生成工具 (v14.29)<br /> *仅在使用 VS 2022/MSBuild 17.0 生成 v142 平台工具集项目时必需*<br /><br /> *可选组件：*适用于 Windows (12.0.0 - x64/x86) 的 C++ Clang 工具 <br /> *仅在使用 Clang 工具集生成时必需* | Microsoft.VisualStudio.Workload.VCTools<br /> Microsoft.VisualStudio.Component.Windows10SDK.19041<br />Microsoft.VisualStudio.Component.Windows11SDK.22000<br /><br /> Microsoft.VisualStudio.Component.VC.Tools.x86.x64<br /><br /> *可选：*Microsoft.VisualStudio.Component.VC.v141.x86.x64<br /><br /> *可选，仅限 VS 2022：* Microsoft.VisualStudio.ComponentGroup.VC.Tools.142.x86.x64<br /><br /> *可选：* Microsoft.VisualStudio.ComponentGroup.NativeDesktop.Llvm.Clang |

| |
|---|
| > 默认情况下，BWOIExample 项目使用 v142 工具集，这意味着它需要 VS 2019 或 VS 2022。 使用 VS 2019 生成需要 MSVC v142 - VS 2019 C++ x64/x86 生成工具（最新）组件，和 VS 2022 需要 MSVC v142 - VS 2019 C++ x64/x86 生成工具 (v14.29) 组件。 |


# 设置生成环境

安装软件要求后，可以设置无需安装的提取 GDK。 可通过两种方式来执行此操作。 如果需要，也可以提取 Windows 10 SDK。

***请注意，VS 2022 支持需要 2022 年 3 月或更高版本的 GDK。***
