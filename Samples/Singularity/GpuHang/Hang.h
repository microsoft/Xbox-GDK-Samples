...
public:
    ...
    virtual void Render(ID3D12Device* device, ID3D12CommandQueue* commandQueue, UINT vendorId, bool hang) = 0;
...
public:
    ...
    void GpuFullStop(ID3D12Device* device, ID3D12CommandQueue* commandQueue) override
    {
        ...
        DX::ThrowIfFailed(device->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(fence.ReleaseAndGetAddressOf())));
...
#elif defined(__GAMING_XBOX)
                device->ReportGpuHangX(D3D12XBOX_REPORT_GPU_HANG_FLAG_NONE);
#else
                OutputDebugString(L"GPU hang detected by title (not expected).\n");

                // Optionally break into the debugger
                static auto breakOnTimeout = false;
                if(breakOnTimeout && IsDebuggerPresent())
                    __debugbreak();
#endif
            }
            else
