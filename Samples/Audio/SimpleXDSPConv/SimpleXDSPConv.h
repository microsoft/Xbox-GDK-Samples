...
constexpr int IR_NUM = 2;
constexpr int MAX_BUFFER_COUNT = 3;
constexpr int MAX_BUFFER_INCREMENT = (MAX_BUFFER_COUNT + 3) & ~0x3;
...
struct BufferManager
{
    ...
    float* outBuffer[MAX_BUFFER_COUNT * IR_NUM];
    ...
    void SetBuffers(float** inBuffers, float** outBuffers)
    {
        ...
        memcpy(outBuffer, outBuffers, MAX_BUFFER_COUNT * IR_NUM * sizeof(float*));
        firstAvailable = 0;
        lock.unlock();
    }
...
private:
    ...
    XDspStreamHandle                            m_dspStreamHandle[IR_NUM];
    XDspStatus*                                 m_dspStatus[IR_NUM];
    float*                                      m_dspBuffer;

    uint32_t                                    m_lastBufferedCommandSequence[IR_NUM];
    uint32_t                                    m_lastRetrievedCommandSequence[IR_NUM];
    uint32_t                                    m_totalQueuedBuffers;
    uint32_t                                    m_totalFinishedBuffers;
    ...
    std::unique_ptr<uint8_t[]>                  m_irFile[IR_NUM];
    DX::WAVData                                 m_irData[IR_NUM];
