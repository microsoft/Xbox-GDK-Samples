...
void GamePlayScreen::DrawHud()
{
    ...
    if (g_game->IsGameWon())
    {
        ...
        else if (g_game->GetWinnerName() == "Host Disconnected")
        {
            winMessageLine1 = "Host has disconnected from the game.";
        }
        else
        {
        ...
        auto speechString = winMessageLine1 + winMessageLine2;
        Managers::Get<SpeechManager>()->RenderTextToSpeech(speechString);
