...
void World::DeserializeShipSpawn(const std::vector<uint8_t>& data)
{
    ...
    DEBUGLOG("DeserializeShipSpawn() received Peerid %u at (%f, %f)\n", peerid, position.x, position.y);
...
std::vector<uint8_t> World::SerializeWorldSetup(std::map<uint64_t, std::shared_ptr<Ship>>& ships) const
{
    ...
    for (auto& activeShipPair : ships)
    {
        ...
        DEBUGLOG("SerializeWorldSetup() sending Peer %llu at (%f, %f) with weapon %u\n", peerid, ship->Position.x, ship->Position.y, currentWeaponType);
    }
...
{
    //Managers::Get<OnlineManager>()->UpdateStatistic("games_started", 1);
    ...
    for (uint32_t i = 0; i < memberSize; ++i)
    {
        ...
        DEBUGLOG("DeserializeWorldSetup() received Peerid %u at (%f, %f) with weapon %u\n", peerid, position.x, position.y, currentWeaponType);
...
{
    DEBUGLOG("DeserializeShipDeath() received for peer %u\n", peerid);
    ...
    if (killerid != 0)
    {
        ...
        if (killerState)
        {
            ...
            {
                /*metadata[L"Location"][L"X"] = killerShip->Position.x;
                metadata[L"Location"][L"Y"] = killerShip->Position.y;
                metadata[L"Target"][L"Location"][L"X"] = shipKilled->Position.x;
                metadata[L"Target"][L"Location"][L"Y"] = shipKilled->Position.y;*/

                Managers::Get<OnlineManager>()->UpdateStatistic("enemies_defeated", 1);
            }
        }
    }
...
void World::DeserializeGameOver(const std::vector<uint8_t> &data)
{
    ...
    //auto online = Managers::Get<IOnlineManager>();

    //online->UpdateStatistic("games_completed", 1);

    if (WinnerName == g_game->GetLocalPlayerName())
    {
        auto state = g_game->GetLocalPlayerState();
        if (state)
        {
            //metadata[L"Score"] = state->GetShip()->Score;
            //online->UpdateStatistic("total_wins", 1);
        }
    }
