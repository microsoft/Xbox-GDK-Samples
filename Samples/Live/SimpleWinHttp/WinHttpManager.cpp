...
#pragma warning(suppress : 6387) // nullptr is valid for WINHTTP_OPTION_UPGRADE_TO_WEB_SOCKET
    ...
    if (context->RequestHeaders.size() > 0)
    {
        auto err = WinHttpAddRequestHeadersEx(
            ...
            0,
            static_cast<DWORD>(context->RequestHeaders.size()),
            context->RequestHeaders.data()
            );
    ...
    {
        std::vector<WINHTTP_EXTENDED_HEADER> winhttpHeaders;

        winhttpHeaders.emplace_back(
        ...
        auto err = WinHttpAddRequestHeadersEx(
            ...
            WINHTTP_EXTENDED_HEADER_FLAG_UNICODE,
            0,
            static_cast<DWORD>(winhttpHeaders.size()),
            winhttpHeaders.data()
            );

        if (err != ERROR_SUCCESS)
        {
            DisplayMessage("Error during WinHttpAddRequestHeadersEx: " + std::to_string(err));
