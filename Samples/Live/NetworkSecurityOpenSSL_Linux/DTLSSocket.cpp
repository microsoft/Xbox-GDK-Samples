...
namespace ATG
{
    ...
    HRESULT DTLSSocket::Create(uint16_t port, XTaskQueueHandle queue, std::string_view certFile, std::string_view keyFile, DTLSSocket** socket) noexcept
    {
        ...
        auto result = SSL_CTX_use_certificate_file(dtlsSocket->m_sslContext, std::string(certFile).c_str(), SSL_FILETYPE_PEM);
        if(result != 1) 
        {
            std::cerr << "DTLSSocket::Create: Unable to load certificate: " << certFile << std::endl;
            return E_INVALIDARG;
        }

        // Load private key file
        result = SSL_CTX_use_PrivateKey_file(dtlsSocket->m_sslContext, std::string(keyFile).c_str(), SSL_FILETYPE_PEM);

        if(result != 1)
        {
            std::cerr << "DTLSSocket::Create: Unable to load private key: " << keyFile << std::endl;
            return E_INVALIDARG;
        }

        // Check if the private key is valid
        result = SSL_CTX_check_private_key(dtlsSocket->m_sslContext);

        if(result != 1)
        {
            std::cerr << "DTLSSocket::Create: Private key check failed!" << std::endl;
            return E_INVALIDARG;
        }
        ...
        auto hr = UdpSocket::Create(port, &internalSocket);
