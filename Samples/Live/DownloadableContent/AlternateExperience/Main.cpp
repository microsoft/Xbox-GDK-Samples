...
LPCWSTR g_szAppName = L"AlternateExperience";
...
int WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE, _In_ LPWSTR lpCmdLine, _In_ int nCmdShow)
{
    ...
    // Change current directory where is same location as launched executable.
    char dir[_MAX_PATH] = {};
    if (GetModuleFileNameA(nullptr, dir, _MAX_PATH) > 0)
    {
        std::filesystem::path exe = dir;
        exe.remove_filename();
        std::ignore = SetCurrentDirectoryA(exe.generic_string<char>().c_str());
    }
    ...
    // Dialog shows to give a chance to attach debugger.
#if 1
#ifdef _GAMING_DESKTOP
    std::ignore = MessageBoxW(nullptr, L"Attach debugger now!", g_szAppName, MB_ICONINFORMATION | MB_OK);
#else
    ...
    if (SUCCEEDED(XGameUiShowMessageDialogAsync(ab, u8"AlternateExperience", u8"Attach debugger now!",
        u8"Done", nullptr, nullptr,
        XGameUiMessageDialogButton::First,
        XGameUiMessageDialogButton::First)) &&
        SUCCEEDED(XAsyncGetStatus(ab, true)) &&
        SUCCEEDED(XGameUiShowMessageDialogResult(ab, &button)))
    {
        OutputDebugString(L"Attach debugger now!\n");
    }
#endif
#endif
...
#endif
    {
        ...
        wcex.lpszClassName = L"AlternateExperienceWindowClass";
        ...
        HWND hwnd = CreateWindowExW(0, L"AlternateExperienceWindowClass", g_szAppName, WS_OVERLAPPEDWINDOW,
