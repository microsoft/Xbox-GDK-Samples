...
void Sample::Initialize(HWND window, int width, int height)
{
    ...
    auto layout = m_uiManager.LoadLayoutFromFile("Assets\\AlternateUILayout.json");
...
void Sample::Update(DX::StepTimer const& timer)
{
    ...
    else if (m_gamePadButtons.a == GamePad::ButtonStateTracker::PRESSED || kb.A)
    {
        ...
        std::filesystem::path exePath = GetCommandLineA();
#else
        std::string launchParam = GetCommandLineA();
        std::string ownExe = "AlternateExperience.exe";
        launchParam.erase(0, launchParam.find(ownExe) + ownExe.length() + 1);
        std::filesystem::path exePath = launchParam;
#endif
        exePath /= "DownloadableContent.exe";

        // The debugger will disconnect here as the current process is ending and a new one is spawned
        XLaunchNewGame(exePath.generic_string<char>().c_str(), nullptr, nullptr);
    }
