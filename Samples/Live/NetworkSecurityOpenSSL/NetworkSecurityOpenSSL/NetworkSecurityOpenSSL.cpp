...
#include "pch.h"
#include "NetworkSecurityOpenSSL.h"
    ...
    m_frame(0)
{
    ...
    m_liveInfoHUD = std::make_unique<ATG::LiveInfoHUD>("NetworkSecurityOpenSSL");
}
...
void Sample::Initialize(HWND window, int width, int height)
{
    ...
    m_acceptConnectionsButton = m_uiManager.FindTypedById<UIButton>(ID("AcceptConnectionButton"));
    ...
    m_acceptConnectionsButton->SetEnabled(false);
...
void Sample::HandleNetworkInitializationComplete()
{
    ...
    async->callback = [](XAsyncBlock* async)
    {
        ...
        uint16_t port = 0;
        auto hr = XNetworkingQueryPreferredLocalUdpMultiplayerPortAsyncResult(async, &port);

        if (SUCCEEDED(hr))
        {
            ATG::DTLSSocket* socket{};
            hr = ATG::DTLSSocket::Create(port, sample->m_taskQueue, &socket);

            if (SUCCEEDED(hr))
            {
                sample->m_socket.reset(socket);
                ...
                sample->Log("Local Address: %s", ATG::AddressToString(localAddress).c_str());

                sample->m_acceptConnectionsButton->SetEnabled(true);
                ...
                sample->m_uiManager.SetFocus(sample->m_openConnectionButton);

                sample->PrintLocalIdentityString();
            }
            else
            {
                sample->Log("Failed to create socket: 0x%08x", hr);
            }
        }
        else
        {
            sample->Log("XNetworkingQueryPreferredLocalUdpMultiplayerPortAsyncResult failed: 0x%08x", hr);
        }

        sample->m_asyncOpWidget->Hide();
    };

    auto hr = XNetworkingQueryPreferredLocalUdpMultiplayerPortAsync(async.get());
    ...
    {
        Log("XNetworkingQueryPreferredLocalUdpMultiplayerPortAsync failed: 0x%08x", hr);
    }
...
void Sample::InitializeUIEventHandlers()
{
    ...
    m_acceptConnectionsButton->ButtonState().AddListenerWhen(UIButton::State::Pressed, [this](UIButton*)
    {
        OnAcceptConnectionsButtonPressed();
    });
    ...
    m_closeConnectionButton->ButtonState().AddListenerWhen(UIButton::State::Pressed, [this](UIButton*)
    {
        ...
        m_acceptConnectionsButton->SetEnabled(true);
...
void Sample::OpenConnection()
{
    ...
    async->callback = [](XAsyncBlock* async)
    {
        ...
        auto hr = ATG::DTLSSocket::CreateConnectionAsyncResult(async, &connection);
        ...
        if (SUCCEEDED(hr))
        {
            ...
            sample->m_acceptConnectionsButton->SetEnabled(false);
        ...
        {
            sample->Log("Failed to create connection: 0x%08x", hr);
...
#pragma region Accept Connections
void Sample::OnAcceptConnectionsButtonPressed()
{
    m_socket->AcceptConnections([](ATG::DTLSSocket* socket, const SOCKADDR* source, const ATG::SocketPayload* data, void* context)
    {
        auto sample = reinterpret_cast<Sample*>(context);

        // Sample only supports one active connection for simplicity
        if (sample->m_connection != nullptr)
        {
            return;
        }

        sample->Log("Incoming connection from %s. Establishing...\n", ATG::AddressToString(*source).c_str());
        ...
        async->callback = [](XAsyncBlock* async)
        {
            ...
            auto hr = ATG::DTLSSocket::AcceptConnectionAsyncResult(async, &sample->m_connection);
            ...
            if (SUCCEEDED(hr))
            {
                ...
                // For sample purposes we don't want to accept any more connections
                sample->m_socket->AcceptConnections(nullptr, nullptr);
            }
            else
            {
                sample->Log("Connection failed. 0x%08x", hr);
            }
        };

        auto hr = socket->AcceptConnectionAsync(source, sample->m_expectedIdentityString, data, async.get());
    ...
    // Xbox console does not allow unsolicited packets to be received the Game OS. We need to send a packet to the IP:Port
    // that we expect to connect to allow incoming packets through the firewall.
    auto asyncBlock = std::make_unique<XAsyncBlock>();
    asyncBlock->queue = m_taskQueue;
    asyncBlock->context = this;
    asyncBlock->callback = [](XAsyncBlock* asyncBlock)
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            sample->GetExpectedIdentityString_AcceptConnections();
        }
    };
    ...
    HRESULT hr = XGameUiShowTextEntryAsync(
        asyncBlock.get(),
        "Network Security Sample",
        "Enter an IP address to allow connections from.",
        "",
        XGameUiTextEntryInputScope::Alphanumeric,
        1024
    );
...
void Sample::GetExpectedIdentityString_AcceptConnections()
{
    auto asyncBlock = std::make_unique<XAsyncBlock>();
    asyncBlock->queue = m_taskQueue;
    asyncBlock->context = this;
    asyncBlock->callback = [](XAsyncBlock* asyncBlock)
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            auto destination = ATG::AddressFromString(sample->m_ipAddr);

            sample->m_socket->AllowConnectionFrom(&destination);
        }
    };
    ...
    asyncBlock->callback = [](XAsyncBlock* asyncBlock)
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            async->callback = [](XAsyncBlock* asyncBlock)
            {
                ...
                auto hr = XAsyncGetStatus(asyncBlock, false);

                sample->Log("Result 0x%08X", hr);
            };

            ATG::SocketPayload data{};

            data.size = used;
            memcpy(data.payload, buffer.get(), used);
...
void Sample::OnCloseConnectionButtonPressed()
{
    ...
    else if (m_acceptingConnections)
    {
        m_socket->AcceptConnections(nullptr, nullptr);
    }
...
void Sample::Update(DX::StepTimer const& timer)
{
    ...
    if (m_socket)
    {
        ...
        do
        {
            ...
            if (SUCCEEDED(hr))
            {
                Log("Message from connection %p: %s", source, std::string(reinterpret_cast<const char*>(data.payload), data.size).c_str());
            }
            else if (hr == E_ABORT)
            {
