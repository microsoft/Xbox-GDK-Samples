...
const char* gameSessionInstructions = "Press A to toggle ready, Y to invite, LB/RB to change ship color/style, Press B to exit";
...
void GameLobbyScreen::ChangeShipColor()
{
...
void GameLobbyScreen::ChangeShipDesign()
{
    std::shared_ptr<PlayerState> localPlayerState = g_game->GetLocalPlayerState();
...
void GameLobbyScreen::Update(float totalTime, float elapsedTime, bool otherScreenHasFocus, bool coveredByOtherScreen)
{
    ...
    if (!m_exiting)
    {
        ...
        switch (state)
        {
        case GameState::MPHostGame:              m_menuEntries.emplace_back("Starting host session...");            break;
        case GameState::MPMatchmaking:           m_menuEntries.emplace_back("Matchmaking: Waiting for players..."); break;
        case GameState::MPMatchmakingCancel:     m_menuEntries.emplace_back("Matchmaking: Canceling...");           break;
        case GameState::ExitingGame:             m_menuEntries.emplace_back("Leaving game...");                     break;
        case GameState::MPJoinGame:              m_menuEntries.emplace_back("Joining Game...");                     break;
        case GameState::EvaluatingNetwork:       m_menuEntries.emplace_back("Evaluating Network...");               break;
        case GameState::MigratingNetwork:        m_menuEntries.emplace_back("Migrating network...");                break;
        case GameState::WaitingForPeerMigration: m_menuEntries.emplace_back("Waiting for peers...");                break;
        case GameState::Lobby:                   HandleLobbyGameState(state);                                       break;
        case GameState::StartingGame:            HandleStartingGameGameState(elapsedTime);                          break;
        }
    }
...
void GameLobbyScreen::HandleLobbyGameState(GameState state)
{
    if (m_lastState != state)
    {
        Managers::Get<OnlineManager>()->SendGameMessage(GameMessage(GameMessageType::PlayerInfo, g_game->GetLocalPlayerState()->DisplayName));
    ...
    if (Managers::Get<OnlineManager>()->IsHost())
    {
        ...
        else if (SampleConfig::c_AllowSinglePlayerMatch)
        {
            allReady = true; //For debugging purposes
        }
        else
        {
            allReady = false;
...
void GameLobbyScreen::HandleStartingGameGameState(float elapsedTime)
{
...
void GameLobbyScreen::SetPlayerReady(bool bPlayerReady)
{
    m_ready = bPlayerReady;
    g_game->GetLocalPlayerState()->LobbyReady = bPlayerReady;

    SendPlayerStateMessage();
}

void GameLobbyScreen::SendPlayerStateMessage()
{
    std::shared_ptr<PlayerState> localPlayerState = g_game->GetLocalPlayerState();

    GameMessage msg{ GameMessageType::PlayerState, localPlayerState->SerializePlayerStateData() };

    Managers::Get<OnlineManager>()->SendGameMessage(msg);
}
