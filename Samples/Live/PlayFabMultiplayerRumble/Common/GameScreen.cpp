...
void GameScreen::HandleInput()
{
    ...
    if (Managers::Get<PlayFabPartyManager>()->IsConnected())
    {
        if (inputManager->IsNewButtonPress(InputManager::GamepadButtons::View) || inputManager->IsNewKeyPress(DirectX::Keyboard::Space))
        {
            ...
            {
                uint32_t textSize = 0;
                HRESULT hr = XGameUiShowTextEntryResultSize(async, &textSize);
                if (SUCCEEDED(hr))
                {
            ...
            HRESULT hr = XGameUiShowTextEntryAsync(&asyncHelper->asyncBlock, nullptr, nullptr, nullptr, XGameUiTextEntryInputScope::Alphanumeric, 255);
            if (FAILED(hr))
            {
