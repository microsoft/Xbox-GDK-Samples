...
void MainMenuScreen::Update(float totalTime, float elapsedTime, bool otherScreenHasFocus, bool coveredByOtherScreen)
{
        ...
        Managers::Get<InputManager>()->IsNewKeyPress(Keyboard::Keys::Y))
    {
        ...
        auto asyncHelper = new ATG::AsyncHelper(asyncQueue.get(), [](XAsyncBlock* async)
        {
            HRESULT hr = XGameUiShowPlayerProfileCardResult(async);
            if (FAILED(hr))
            {
                LogError_HRESULT("XGameUiShowPlayerProfileCardResult", hr);
            }
        });

        XboxUser* user = Managers::Get<XboxUserManager>()->GetCurrentUser();
        XUserHandle userHandle = *user;

        HRESULT hr = XGameUiShowPlayerProfileCardAsync(&asyncHelper->asyncBlock, userHandle, user->Id());
        if (FAILED(hr))
        {
            LogError_HRESULT("XGameUiShowPlayerProfileCardAsync", hr);
            delete asyncHelper;
        }
    }
}
...
void MainMenuScreen::MainMenu()
{
    ...
    if (Managers::Get<OnlineManager>()->HasMultiplayerPrivileges())
    {
        if (Managers::Get<OnlineManager>()->HasCrossplayPrivileges())
        {
            ...
            m_menuEntries.emplace_back("BROWSE", []()
            {
                Managers::Get<ScreenManager>()->AddGameScreen<LobbyBrowserMenu>();
            });
        }
        else
        {
            m_menuEntries.emplace_back("Your account does not have crossplay privileges");
        }
    }
    else
    {
