...
LPCWSTR g_szAppName = L"NetworkSecurityBCrypt";
...
int SampleMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE, _In_ LPWSTR lpCmdLine, _In_ int nCmdShow)
{
...
#endif
    {
        ...
        wcex.lpszClassName = L"NetworkSecurityBCryptWindowClass";
        ...
        HWND hwnd = CreateWindowExW(0, L"NetworkSecurityBCryptWindowClass", g_szAppName, WS_OVERLAPPEDWINDOW,
        ...
        int success = WSAStartup(MAKEWORD(2, 2), &wsa);
        if (success != 0)
        {
            g_sample->Log("Failed to initialize Winsock with result = (0x%08X)", HRESULT_FROM_WIN32(static_cast<uint32_t>(success)));
            return 1;
        }
