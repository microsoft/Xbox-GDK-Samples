...
namespace ATG
{
    ...
    public:
        ...
        void Init(std::function<uint32_t()> callback, XTaskQueueHandle queue) noexcept
        {
            async.context = this;
            async.queue = queue;
        ...
        {
            std::scoped_lock lock(m_lock);
            return ResetNoLock(newDelayMS);
        }

        HRESULT ResetNoLock(uint32_t newDelayMS) noexcept
        {
    ...
    private:
        ...
        std::function<uint32_t()> m_callback;
