...
namespace DX
{
    ...
    inline void XM_CALLCONV DrawControllerString(_In_ DirectX::SpriteBatch* spriteBatch, _In_ DirectX::SpriteFont* textFont, _In_ DirectX::SpriteFont* butnFont,
        _In_z_ wchar_t const* text, DirectX::XMFLOAT2 const& position, DirectX::FXMVECTOR color = DirectX::Colors::White, float scale = 1, float hdrScale = 1)
    {
            ...
            if (buttonText)
            {
                ...
                if (text[ch] == L']')
                {
                    ...
                    if (*button)
                    {
                        ...
                        outPos.x += offsetX;
                        outPos.y -= offsetY;
                        XMVECTOR hdrColor = XMVectorScale(Colors::White, hdrScale);
                        hdrColor = XMVectorSetW(hdrColor, 1.0f);
                        butnFont->DrawString(spriteBatch, button, outPos, hdrColor, 0.f, XMFLOAT2(0.f, 0.f), XMFLOAT2(buttonScale, buttonScale));
