...
void FullScreenQuad::Initialize(_In_ ID3D12Device* d3dDevice)
{
    ...
    const CD3DX12_DESCRIPTOR_RANGE textureSRVs_3(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 2);
    ...
    rootParameters[RootParameterIndex::TextureSRV_3].InitAsDescriptorTable(1, &textureSRVs_3, D3D12_SHADER_VISIBILITY_PIXEL);

    CD3DX12_ROOT_SIGNATURE_DESC rootSignatureDesc;
    rootSignatureDesc.Init(static_cast<UINT>(std::size(rootParameters)), rootParameters, 2, samplers, rootSignatureFlags);
    ...
    D3D12_GPU_VIRTUAL_ADDRESS constantBuffer)
{
    ...
    d3dCommandList->SetGraphicsRootDescriptorTable(RootParameterIndex::TextureSRV_3, texture);
    ...
    D3D12_GPU_VIRTUAL_ADDRESS constantBuffer)
{
    ...
    d3dCommandList->SetGraphicsRootDescriptorTable(RootParameterIndex::TextureSRV_3, texture2);
    d3dCommandList->SetPipelineState(d3dPSO);
    d3dCommandList->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
    d3dCommandList->DrawInstanced(3, 1, 0, 0);
}
...
void FullScreenQuad::Draw(
    ...
    D3D12_GPU_DESCRIPTOR_HANDLE texture,
    D3D12_GPU_DESCRIPTOR_HANDLE texture2,
    D3D12_GPU_DESCRIPTOR_HANDLE texture3,
    D3D12_GPU_VIRTUAL_ADDRESS constantBuffer)
{
    d3dCommandList->SetGraphicsRootSignature(m_d3dRootSignature.Get());
    d3dCommandList->SetGraphicsRootConstantBufferView(RootParameterIndex::ConstantBuffer, constantBuffer);
    d3dCommandList->SetGraphicsRootDescriptorTable(RootParameterIndex::TextureSRV, texture);
    d3dCommandList->SetGraphicsRootDescriptorTable(RootParameterIndex::TextureSRV_2, texture2);
    d3dCommandList->SetGraphicsRootDescriptorTable(RootParameterIndex::TextureSRV_3, texture3);
    d3dCommandList->SetPipelineState(d3dPSO);
    d3dCommandList->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
    d3dCommandList->DrawInstanced(3, 1, 0, 0);
}
