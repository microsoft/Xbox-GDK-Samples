...
namespace ImGuiAtg
{
    ...
    void DrawLog(const char* title, float filterWidth = -200.0f);
    void DrawLogPanel(float height = 0.0f);
    void ClearLog();
    bool IsLogCollapsed();

    void LogTo(const char* id, const char* fmt, ...);
    void DrawNamedLog(const char* title, const char* id, float filterWidth = -200.0f);
    void DrawNamedLogPanel(const char* id, float height = 0.0f);
