    ...
    , m_displayHeight(0)
{
    ...
    if (IsAvailable())
    {
        ThrowIfFailed(XFrontPanelGetScreenDimensions(&m_displayHeight, &m_displayWidth));

        m_buffer = std::make_unique<uint8_t[]>(m_displayWidth * m_displayHeight);
    }
}
...
void FrontPanelDisplay::Present()
{
    if (IsAvailable())
    {
        XFrontPanelPresentBuffer(m_displayWidth * m_displayHeight, m_buffer.get());
    }
}
