...
public:
    ...
    void Update(float elapsedTime, const GamePad::State& pad)
    {
        ...
        // Translate camera
        {
            ...
            if (move.x != 0 || move.y != 0 || move.z != 0)
            {
                ...
                m_cameraPosition = XMVectorAdd(m_cameraPosition, XMVectorScale(move, elapsedTime * m_sensitivity));
        ...
        {
            const float invert = (m_flags & c_FlagsInvertY) ? -1.f : 1.f;
            ...
            {
                const float upSign = copysignf(1, im.Up().y);
                const Vector3 up = upSign * Vector3::Up;
    ...
    void Update(float elapsedTime, Mouse& mouse, const Keyboard& kb)
    {
        ...
        // Keyboard controls
        {
            ...
            if (move.x != 0 || move.y != 0 || move.z != 0)
            {
                ...
                m_cameraPosition = XMVectorAdd(m_cameraPosition, XMVectorScale(move, elapsedTime));
        ...
        if (mstate.positionMode == Mouse::MODE_RELATIVE)
        {
            if (mstate.rightButton)
            {
                ...
                m_cameraPosition = XMVectorAdd(m_cameraPosition, XMVectorScale(delta, elapsedTime * m_sensitivity));
                m_viewDirty = true;
            }
    ...
    XMMATRIX GetView() const
    {
        ...
        const XMVECTOR dir = XMVector3Rotate((m_lhcoords) ? g_XMIdentityR2 : g_XMNegIdentityR2, m_cameraRotation);
...
void FlyCamera::Update(float elapsedTime, Mouse& mouse, const Keyboard& kb)
{
