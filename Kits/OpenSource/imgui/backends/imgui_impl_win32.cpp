...
// ATG changes
// - X1 and Scarlett compatible
// - GameInput gamepad implementation, XInput removed
// - Secure CRT tweaks
...
#define GAMEINPUT_KEYSTATE_MAX    16
...
static void ImGui_ImplWin32_UpdateGamepads(ImGuiIO& io)
{
    ...
    if(SUCCEEDED(hr))
    {
        ...
        if(reading->GetGamepadState(&state))
        {
            ...
            #define MAP_BUTTON(KEY_NO, BUTTON_ENUM)     { io.AddKeyEvent(KEY_NO, (state.buttons & BUTTON_ENUM) != 0); }
            #define MAP_ANALOG(KEY_NO, VALUE, V0, V1)   { float vn = (float)(VALUE - V0) / (float)(V1 - V0); io.AddKeyAnalogEvent(KEY_NO, vn > 0.10f, IM_SATURATE(vn)); }

            #define GAMEINPUT_GAMEPAD_THUMB_DEADZONE       .24f
            #define GAMEINPUT_GAMEPAD_TRIGGER_THRESHOLD    .11f
            ...
            MAP_ANALOG(ImGuiKey_GamepadL2,              state.leftTrigger,  GAMEINPUT_GAMEPAD_TRIGGER_THRESHOLD, 1.0f);
            MAP_ANALOG(ImGuiKey_GamepadR2,              state.rightTrigger, GAMEINPUT_GAMEPAD_TRIGGER_THRESHOLD, 1.0f);
            ...
            MAP_ANALOG(ImGuiKey_GamepadLStickLeft,      state.leftThumbstickX,  -GAMEINPUT_GAMEPAD_THUMB_DEADZONE, -1.0f);
            MAP_ANALOG(ImGuiKey_GamepadLStickRight,     state.leftThumbstickX,  +GAMEINPUT_GAMEPAD_THUMB_DEADZONE, +1.0f);
            MAP_ANALOG(ImGuiKey_GamepadLStickUp,        state.leftThumbstickY,  +GAMEINPUT_GAMEPAD_THUMB_DEADZONE, +1.0f);
            MAP_ANALOG(ImGuiKey_GamepadLStickDown,      state.leftThumbstickY,  -GAMEINPUT_GAMEPAD_THUMB_DEADZONE, -1.0f);
            MAP_ANALOG(ImGuiKey_GamepadRStickLeft,      state.rightThumbstickX, -GAMEINPUT_GAMEPAD_THUMB_DEADZONE, -1.0f);
            MAP_ANALOG(ImGuiKey_GamepadRStickRight,     state.rightThumbstickX, +GAMEINPUT_GAMEPAD_THUMB_DEADZONE, +1.0f);
            MAP_ANALOG(ImGuiKey_GamepadRStickUp,        state.rightThumbstickY, +GAMEINPUT_GAMEPAD_THUMB_DEADZONE, +1.0f);
            MAP_ANALOG(ImGuiKey_GamepadRStickDown,      state.rightThumbstickY, -GAMEINPUT_GAMEPAD_THUMB_DEADZONE, -1.0f);
            #undef MAP_BUTTON
            #undef MAP_ANALOG
        }
    }
#endif // #ifndef IMGUI_IMPL_GDK_DISABLE_GAMEPAD
}
...
// See https://learn.microsoft.com/windows/win32/tablet/system-events-and-mouse-messages
...
float ImGui_ImplWin32_GetDpiScaleForMonitor(void* monitor)
{
    ...
    xdpi = static_cast<UINT>(::GetDeviceCaps(dc, LOGPIXELSX));
    ydpi = static_cast<UINT>(::GetDeviceCaps(dc, LOGPIXELSY));
    IM_ASSERT(xdpi == ydpi); // Please contact me if you hit this assert!
